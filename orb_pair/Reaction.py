...
from chemutils.Common.OrbitalModel import (
    Orbital as chemutilsOrbital,
    clearNonInvolvedAtomMaps,
)
from chemutils.CombiCDB.Const import REACTION_DELIM
from chemutils.CombiCDB.MarvinArrowLookup import arrowToOrbital
...
class Reaction:
    ...
    def __init__(self, smirks, arrows, *args, **kwargs):
        if ">>" in smirks:
            ...
            raise ValueError("do not label reagents.")
            log.error(
                "If the reaction is in correct format, you probably are including the reagents."
            )
        ...
        reactive_products = [
            i for i in products_smiles if i not in self.spectators.split(".")
        ]
    ...
    def aromatic_atm_map_smiles(self):
        ...
        return clean_atom_map_smiles(
            self.atm_map_reactant_smiles, doKekule=False, doArom=True
        )
    ...
    def find_spectators(reactants, products):
        ...
        return (
            smi_to_unique_smi_fast(specs_smiles),
            smi_to_unique_smi_fast(reactant_smiles),
            smi_to_unique_smi_fast(product_smiles),
        )
    ...
    def toArrowCodes(self):
        ...
        arrowObjList = []
        bond = moveOrbitalElectrons(
            elemStepData["filledOrb"],
            elemStepData["unfilledOrb"],
    ...
    def __unicode__(self):
        ...
        """
        complete = (
            hasattr(self, "src_orbital")
            and hasattr(self, "sink_orbital")
            and self.iso_product_smiles
        )
    ...
    def cat1(self):
        """
        Currently we are reading the category 1 from the file.
        ltimately, this must be implemented here so we extract the
        category 1 from the Reaction object
        """
        cat1 = "unknown"
        self.cat1 = cat1
        return cat1
    ...
    def cat2(self):
        """
        Currently we are reading the categor 2 from the file.
        ltimately, this must be implemented here so we extract the
        category 2 from the Reaction object
        """
        cat2 = "unknown"
        self.cat2 = cat2
        return cat2
    ...
    def setup_reaction(self, atm_map_reactant_smiles, arrowCodes, *args, **kwargs):
        ...
        srcOrbStr, sinkOrbStr, srcOrb, sinkOrb, nElectrons = arrowToOrbital(
            atm_map_reactant_smiles, arrowCodes
        )
        ...
        atm_map_reactant_smiles = getAtmMappedSmilesFromCompositeSmiles(
            initSmi, clearMaps=False
        )
        ...
        nSrc = chemutilsOrbital.fromLabeledMolAndInfoStr(
            newOEMol, srcOrb.formatInfoStr()
        )
        nSink = chemutilsOrbital.fromLabeledMolAndInfoStr(
            newOEMol, sinkOrb.formatInfoStr()
        )
        ...
        srcAtmSmi = getAtmMappedSmilesFromCompositeSmiles(
            srcAtmSmi, clearMaps=False
        )  # here is where the non-connected components are dropped. part w map is saved

        srcOEMol = molBySmiles(srcAtmSmi)
        singleMolSrcOrb = chemutilsOrbital.fromLabeledMolAndInfoStr(
            srcOEMol, srcInfoStr
        )
        ...
        singleMolSinkOrb = chemutilsOrbital.fromLabeledMolAndInfoStr(
            sinkOEMol, sinkInfoStr
        )
        ...
        iso_product_smiles = standardizeSmiles(
            clearAtomMapsSmiStr(productSmi), aromatize=True
        )
    ...
    def get_or_create_from_object(cls, obj):
        ...
        return cls.objects.get_or_create(
            atm_map_reactant_smiles=obj.atm_map_reactant_smiles,
