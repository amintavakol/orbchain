...
class SimpleOrbPairObject:
    ...
    def __init__(
        self, srcOrb, sinkOrb, srcAtom, sinkAtom, radical=False, pericyclic=False
    ):
        """Constructor"""
        self.srcOrb = srcOrb
        ...
        self.reactionSmiles = reactionSmilesFromOrbitalPair(
            self.srcOrb, self.sinkOrb, False, self.arrowObjectList
        )
        self.reactantSmiles, self.productSmiles = self.reactionSmiles.split(
            REACTION_DELIM
        )
    ...
    def orbPairObjectsFromAtoms(
        ...
        if radical:
            proposer = RadicalProposer(
                srcAtomList, sinkAtomList, photo=photo, checkIntra=checkIntra
            )
        elif pericyclic:
    ...
    def sum_atom_pred_values(self):
        if not self.atom_pred_vals_sum:
            self.atom_pred_vals_sum = (
                self.srcAtom.fill_pred_value + self.sinkAtom.unfill_pred_value
            )
