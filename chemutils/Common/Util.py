...
try:
    ...
    print(sys.stderr, "OEChem dependency does not appear to be installed.  Attempting to continue anyway.")
    # Insert a couple fake values so that later calls won't crash completely (though they won't work properly either)
    oemolistream = list;
    oemolostream = list;
    OEFormat_SMI = -1;

from chemutils.Common.Const import FP_MAX, FP_MIN, FP_SIZE, SMILES_MOL_DELIM, REACTION_COMPONENT_DELIM;
from chemutils.Common.MolStdValue import BOLTZMANN_CONST, PLANCKS_CONST, ATOM_PER_MOL, ROOM_TEMPERATURE;
...
def generateFingerprint(smiles,fpMax=FP_MAX,fpMin=FP_MIN,fpSize=FP_SIZE):
    ...
    if fingerprint is None:
        ...
        query = urllib.quote_plus("ENCODE %s %d %d %d\n" % (smiles,fpMax,fpMin,fpSize) );
