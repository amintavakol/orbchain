...
from openeye.oechem import OEBondStereo_CisTrans, OEBondStereo_Trans
from openeye.oechem import (
...
from chemutils.Common.MolStdValue import (
    stableValenceShell,
    nStdValenceElectrons,
    nMaxHyperValenceElectrons,
    STD_OCTECT,
)
from chemutils.Common.MolStdValue import PERIODIC_ROW
from chemutils.Common.Const import SMILES_MOL_DELIM
from chemutils.Common.Util import createAtomMapSmiString, log
from chemutils.Common.Util import atomByMapIdx
...
"""
ORBITAL_TYPES = set(
    ["sigma", "pi", "s", "sp0", "sp", "sp1", "sp2", "sp3", "p", "sp3d", "d"]
)
...
class Orbital:
    ...
    def __init__(
        self, atom, type, bond, electrons, mol=None, atomFaceList=None, nbrFaceList=None
    ):
        if type is None:
        ...
        if type not in ORBITAL_TYPES:
            if type in MOCK_ORBITAL_TYPES:
                log.warning(
                    "Using mock orbital type: %s - %s (%s)"
                    % (type, createAtomMapSmiString(mol), atom.GetMapIdx())
                )
            else:
                # Not even a mock orbital, who knows what the caller is trying to produce
                raise Exception(
                    "Unexpected orbital type: %s - %s (%s)"
                    % (type, createAtomMapSmiString(mol), atom.GetMapIdx())
                )
    ...
    def setSynAdditionFlags(self):
        ...
        """
        if not self.isBondOrbital() or (
            self.atomFaceList is None and self.nbrFaceList is None
        ):
            self.__flagIsSynAddition = False
        ...
        if self.type == "sigma":
            if not self.atom.HasStereoSpecified() or self.atomFaceList is None:
                ...
                self.__flagIsSynAddition = (
                    self.atom.GetStereo(tempAtomList, OEAtomStereo_Tetrahedral)
                    == OEAtomStereo_Left
                )
            ...
            if not self.neighbor.HasStereoSpecified() or self.nbrFaceList is None:
                ...
                self.__flagIsNbrSynAddition = (
                    self.neighbor.GetStereo(tempAtomList, OEAtomStereo_Tetrahedral)
                    == OEAtomStereo_Left
                )
            ...
            if (
                self.atomFaceList is None
                or self.nbrFaceList is None
                or not self.getBond().HasStereoSpecified()
            ):
                self.__flagIsSynAddition = False
                return
            bondAtomIdx = -1
            ...
            self.__flagIsSynAddition = (
                self.getBond().GetStereo(
                    [atomFaceList[0], nbrFaceList[0]], OEBondStereo_CisTrans
                )
                == OEBondStereo_Trans
            )
    ...
    def fromLabeledMol(
                ...
                if atomFaceLabelList is not None:
                    ...
                    for label in atomFaceLabelList:
                        if label in mapIdxDict:
                            ...
                            log.warning(
                                "Unable to find an face atom with label: %s" % label
                            )
                            atomFaceAtomList = None
                            break
                            ...
                            if nbrFaceLabelList is not None:
                                ...
                                for label in nbrFaceLabelList:
                                    if label in mapIdxDict:
                                        ...
                                        # Unable to find an atom matching the label, must be a bogus specification, ignore it then
                                        log.warning(
                                            "Unable to find an face atom with label: %s"
                                            % label
                                        )
                                        nbrFaceAtomList = None
                                        break
        ...
        if orbAtom is not None:
            orbital = Orbital(
                ...
                electrons,
                mol,
                atomFaceList=atomFaceAtomList,
                nbrFaceList=nbrFaceAtomList,
            )
    ...
    def isMultipleComponent(self):
        ...
        involvedAtomParts = set(
            [parts[atm.GetIdx()] for atm in self.involvedAtomList()]
        )
    ...
    def fromLabeledMolAndInfoStr(mol, orbInfoStr):
        ...
        if orbInfoStr is not None:
            ...
            for componentOrbInfoStr in orbInfoStrList:
                tokens = (
                    componentOrbInfoStr.split()
                )  # Assume simple white-space separation
    ...
    def fromLabeledMolAndOrbital(mol, baseAtomLabel, templateOrb):
        ...
        newOrb = Orbital.fromLabeledMol(
            mol,
            baseAtomLabel,
            templateOrb.type,
            baseAtomLabel + 1,
            templateOrb.electrons,
            atomFaceLabelList,
            nbrFaceLabelList,
        )

        if templateOrb.extOrbital is not None:
            newOrb.extOrbital = Orbital.fromLabeledMolAndOrbital(
                mol, templateOrb.extOrbital.atom.GetMapIdx(), templateOrb.extOrbital
            )
    ...
    def cloneOrbital(originalOrb, copyMol, copyAtomsByIndex):
        ...
        if originalOrb.atomFaceList is not None:
            copyAtomFaceList = [
                copyAtomsByIndex[atm.GetIdx()] for atm in originalOrb.atomFaceList
            ]
        if originalOrb.nbrFaceList is not None:
            copyNbrFaceList = [
                copyAtomsByIndex[atm.GetIdx()] for atm in originalOrb.nbrFaceList
            ]
        ...
        copyOrb = Orbital(
            ...
            originalOrb.type,
            copyBond,
            originalOrb.electrons,
            mol=copyMol,
            atomFaceList=copyAtomFaceList,
            nbrFaceList=copyNbrFaceList,
        )

        if originalOrb.extOrbital is not None:
            # Extended orbital chain, recursively continue cloning
            copyExt = Orbital.cloneOrbital(
                originalOrb.extOrbital, copyMol, copyAtomsByIndex
            )
    ...
    def compositeOrbitalsFromDistinctMols(orbitalList):
        ...
        for iOrb, componentOrb in enumerate(orbitalList):
            orbBaseLabel = ORBITAL_LABEL_INCREMENT * (iOrb + 1)
            log.debug(
                "orbBaseLabel : %d, componentOrb : %s"
                % (orbBaseLabel, str(componentOrb.toLabeledSmiAndInfoStr()))
            )
            # compositeOrb = Orbital.fromLabeledMolAndOrbital( compositeMol, orbBaseLabel, componentOrb );
            compositeOrb = Orbital.fromLabeledMolAndInfoStr(
                compositeMol, orbInfoStrList[iOrb]
            )
            compositeOrbList.append(compositeOrb)
            log.debug(
                "After return compositeOrb : %s"
                % str(compositeOrb.toLabeledSmiAndInfoStr())
            )
    ...
    def __str__(self):
        ...
        if bond is not None:
            ...
            bondStr = "-%s-%s" % (
                bond.GetOrder(),
                OEGetAtomicSymbol(neighborAtom.GetAtomicNum()),
            )
    ...
    def htmlLabel(self, occupied=True):
        ...
        if self.neighbor is not None:
            ...
            bondStr = "%s%s" % (
                bondSymbol,
                OEGetAtomicSymbol(self.neighbor.GetAtomicNum()),
            )
    ...
    def flippedBondOrbital(self):
        ...
        flipOrb.neighbor = self.atom
        # Add after the fact, because cannot count on a bond existing in the current state
...
def orbitalIter(atom, includeRedundant=False, useSymmetries=False):
    ...
    for bond in atom.GetBonds():
        ...
        if includeRedundant or bond.GetOrder() < 2:
            ...
            if (
                includeRedundant
                or (not useSymmetries)
                or (nbrSymClass not in seenSymClassIdx)
            ):
    ...
    for iHydrogen in range(atom.GetImplicitHCount()):
        if includeRedundant or (iHydrogen < 1 and not seenHydrogen):
            ...
            log.debug(
                "About to yield an implicit Hydrogen orbital: %s "
                % str(orb.toLabeledSmiAndInfoStr())
            )
            yield orb
        spCount += 1
    ...
    hybridization = atomHybridization(atom)
    # Guess the atom's hybridization state
    stableShell = stableValenceShell(atom.GetAtomicNum())
    # How many electrons needed to make a stable outer shell?
    ...
    expectedSP = spHybrid + 1
    # Assuming OEChem indexing, expected sp orbitals is the index + 1;  e.g., sp3 should have 4
    expectedSP = min(expectedSP, expectedOrbitals)
    # Hydrogen won't be sp3 really, just base on expected orbitals
    expectedP = expectedOrbitals - expectedSP
    # Assuming 4 orbitals for 8 stable valence electron shell
    expectedD = maxHyperValElectrons / 2
    # Are there potential d orbitals?
...
def bondOrbitalByMapIdx(
    mol, sourceIdx, neighborIdx, piFirst=True, giveDefaultOnMissing=True
):
    ...
    orb = [
        orb
        for orb in orbIter
        if orb.neighbor is not None and orb.neighbor.GetMapIdx() == neighborIdx
    ]
...
def atomOrbitalByMapIdx(mol, atomIdx, numElectrons, giveDefaultOnMissing=True):
    ...
    orb = [
        orb
        for orb in orbIter
        if orb.type not in BOND_ORBITAL_TYPES and orb.electrons == numElectrons
    ]
    if orb == []:
        if giveDefaultOnMissing:
...
def orbitalInfo(atom):
    ...
    nRadicals = (
        nElectrons % 2
    )  # Modulo.  Assume pairs will always try to stick together for lowest energy orbitals
...
def atomHybridization(atom, visitedAtomIndexes=None):
    ...
    if (
        nValenceEle == 2
    ):  # H, Li, Na, K, etc. that are 1 electron more than an empty outer shell
    ...
    return (
        3 - pOrbitals + 10 * dOrbitals
    )  # Default sp3. but discount any p-orbital only bonds / electrons
