...
from chemutils.Common import Const
...
def main(argv):
    ...
    usageStr = (
        "usage: %prog [options] <outputFile> <searchString>\n"
    ...
    parser.add_option(
        ...
        dest="annotation",
        action="store_true",
        help="If set, returns the annotation table for this chemical.",
    )
    parser.add_option(
        "-s",
        "--sdf",
        dest="sdf",
        action="store_true",
        help="If set, returns the SDF 3D file contents for this chemical.",
    )
...
class ChemicalDetail:
    ...
    def chemicalDetails(self, searchColumn, searchValue):
        ...
        results = DBUtil.execute(
            query, (searchValue,), includeColumnNames=True, conn=conn
        )
        ...
        if len(results) > 1:
            ...
            query = (
                "select sdf, isomer from isomer3d where chemical_id = %s"
                % SQL_PLACEHOLDER
            )
    ...
    def annotationDictsBySmiles(self, smiles, limit=None):
        ...
        for i in xrange(len(chemicalmix_ids)):
            chemicalmix_ids[i] = chemicalmix_ids[i][0]
            # Convert lists of size 1, to single items
        if len(chemicalmix_ids) < 1:
            chemicalmix_ids.append(-1)
            # Append dummy value so future queries don't break
        ...
        annotationQuery.addFrom(
            "source2chemicalmix as s2c left join annotation as a on s2c.source2chemicalmix_id = a.source2chemicalmix_id"
        )
    ...
    def separateAnnotationDictsByMetadata(self, annotationDicts, metadataColumn):
        ...
        metadataSet = set()
        # Convert into a Set of tuples for rapid lookup
        for row in metadataResults:
            metadataSet.add(tuple(row))

        filteredDicts = []
        for i in xrange(
            len(annotationDicts) - 1, -1, -1
        ):  # Iterate in reverse order since we may be deleting items
        ...
        if streamOutput:
            ...
            for chemicalModel in chemicalModels:
                if not idInfoOnly:
                    ...
                    chemicalDetailModel = self.chemicalDetails(
                        "iso_smiles", chemicalModel["iso_smiles"]
                    )
                ...
                if (
                    "source.abbrev" in chemicalModel
                    and "source2chemicalmix.external_id" in chemicalModel
                ):
                    for sourceAbbrev, external_id in zip(
                        chemicalModel["source.abbrev"],
                        chemicalModel["source2chemicalmix.external_id"],
                    ):
                        OESetSDData(
                            mol, sourceAbbrev + Const.EXTERNAL_ID_SUFFIX, external_id
                        )
    ...
    def __parseSDFString(mol, sdfString):
        ...
        if success < 0:
            ...
            sourceOEIS = virtual_oemolistream("\n" + sdfString, OEFormat_SDF)
            # Restart, but try adding blank title line
    ...
    def __applySDTags(self, isomerMol, isomerModel):
        ...
        for col, value in isomerModel.iteritems():
            if col not in Const.EXCLUDED_SD_TAGS:
