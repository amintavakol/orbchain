...
class ChemicalSearchModel:
    ...
    def __init__(self):
        ...
        self.similarityThreshold = None;# Minimum similarity a target must have to satisfy similarity search
        self.sourceIds = None;          # ID numbers of the sources to filter by
        self.strictSubstructure = False;# If true and have similarMols to search by, apply them as SMARTS patterns to match strict substructures
        self.includeSourceData = False; # Whether to include info on which sources provide this chemical.  Disable to improve performance
        self.maxResults = None;         # Maximum number of results to return
        self.resultsStart = None;       # Offset of first result to display, usually used in combination with maxResults

        # Special criteria that should added through respective class methods
        self.similarMols = [];          # Molecules to find similarity to
        self.__textCriteria = None;     # Text criteria to search by (e.g., chemical names)
        self.__textField = None;        #   Which field to do the text search by
        self.__discreteCriteria = [];   # Discrete criteria 2-ples to search by.(col,value) pairs
        self.__rangeCriteria = [];      # Range criteria 3-ples to search by.   (col,minValue,maxValue) triples

        # Internal use attributes to facilitate searches.  Public users trying to set them will achieve no effect
        self.substructureFilters = None;# SMARTS based substructure filters when need to do strict searching
        self.similarQuery = None;       # Actual text query string to send similarity search server

        # Output attributes
        self.maxAdvancedResultsExceeded = False; # Results indicator if similarity search results went farther than acceptable
...
#            );
    ...
    def setTextCriteria( self, textValue, field ):
        ...
        if textValue is not None:
            textValue = textValue.strip();  # Dump flanking whitespace
    ...
    def popDiscreteCriteria(self):
        ...
        """
        if len(self.__discreteCriteria) > 0:    return self.__discreteCriteria.pop();
        else:                                   return None;
