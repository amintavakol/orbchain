...
from chemutils.Common.Util import molBySmiles, generateFingerprint
...
class SimpleReactantPool(BaseReactantPool):
    ...
    def __init__(self, reactantSmilesList):
        ...
        for reactantSmiles in reactantSmilesList:
            ...
            self.reactantPool.append(
                (OECreateIsoSmiString(reactantMol), reactantMol, fingerprint)
            )
    ...
    def search(self, queryMol, resultsLimit):
        ...
        for reactantSmi, reactantMol, fingerprint in self.reactantPool:
            score = tversky(queryFP, fingerprint, 1.0, 1.0)
            # Set to 0.1, 0.9 to favor query being a super-structure of reactants
            scoredPool.append((score, reactantMol))
        scoredPool.sort()
        # Sort by score
        scoredPool.reverse()
        # Want it in desc. order
...
class ChemDBReactantPool(BaseReactantPool):
    ...
    def __init__(self, similarityThreshold):
        ...
        # Save on performance, don't want this information
    ...
    def search(self, queryMol, resultsLimit):
        ...
        self.searchModel.similarMols.pop()
    ...
    def __contains__(self, queryMol):
        ...
        chemicalModels = self.chemicalSearch.findChemicals(self.searchModel)
        self.searchModel.popDiscreteCriteria()
        # Revert to original state
    ...
    def __call__(self, retroReagentList, targetMol):
        ...
        for iReagent, retroReagent in enumerate(retroReagentList):
            ...
            for retroProduct in retroProducts:
                ...
                scoredRetroProductsByReagent.append(
                    (aggregateScore, (retroProduct, retroReagent, iReagent))
                )
        scoredRetroProductsByReagent.sort()
        scoredRetroProductsByReagent.reverse()
        # Want desc. order by score

        topRetroProductsByReagent = []
        for iProduct in xrange(
            min(self.returnCount, len(scoredRetroProductsByReagent))
        ):
            topRetroProductsByReagent.append(scoredRetroProductsByReagent[iProduct][1])
            # Copy only the top results, without scores
...
def synthesisTreeTableFromReactionSteps(reactionSteps):
    ...
    rootProductList.sort()
    # Produce in a consistent order
...
def analyzeProductMixture(productList, target=None):
    ...
    productCanSmiSet = set()
    # Non-stereoisomer specific canonical, not isomer SMILES
    ...
    return (
        len(productCanSmiSet),
        len(productIsmSmiSet),
        unintendedProducts,
...
def precursorStatistics(precursorList):
    ...
    for precursor in precursorList:
        ...
        statDict["maxRingSystemCount"] = max(
            ringSystemCount, statDict["maxRingSystemCount"]
        )
