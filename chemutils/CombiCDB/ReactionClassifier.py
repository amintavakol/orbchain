...
from chemutils.Common.Util import stdOpen, virtual_oemolistream, createStandardSmiString;
...
from chemutils.CombiCDB.ReactionFormatter import RDFReader, reactionmolistream;
from chemutils.Common.Util import *;
from openeye.oechem import *;

from Util import log;
from chemutils.Common.test.Util import ChemDBTestCase
...
from chemutils.Common.Util import molBySmiles, splitCompositeMolToSmilesList, createStandardSmiString;
from chemutils.Common.Model import modelDictFromList;
        ...
        for molReaction in reactionDatabaseStream.GetOEGraphMols():
           ...
           # for molPattern in reactionPatternStream.GetOEGraphMols():
            ...
            for linePattern in rpf:
                ...
                if reactionMatcher.runByReaction(molReactionString,molPatternString)==1:
                    ...
                    for linePatternSecond in rpfSecond:
                        molPatternStringSecond,molPatternNameSecond=linePatternSecond.split(" ",1)
                        # judge if two reactions are the same or not
                        if reactionMatcher.runByTwoReactionPatterns(molReactionString,molPatternString,molPatternStringSecond)==True:
                            ...
                            for linePatternThird in rpfThird:
                                molPatternStringThird,molPatternNameThird=linePatternThird.split(" ",1)
                                if reactionMatcher.runByThreeReactionPatterns(molReactionString,molPatternString,molPatternStringSecond,molPatternStringThird)==True:
...
class ReactionMatcher:
    ...
    def runByReaction( self, molReaction, molPattern):
        ...
        generatedProductIS = virtual_oemolistream(generatedProductOEOS.GetString(), OEFormat_ISM);
        # extract products
        ...
        isMatched = compareProducts( expectedProductMol, generatedProductIS.GetOEGraphMols() ,reactantMolList);
        return isMatched;
    ...
    def runByTwoReactionPatterns( self, molReaction, molPattern1, molPattern2):
        ...
        generatedProductIS1 = virtual_oemolistream(generatedProductOEOS1.GetString(), OEFormat_ISM);
        expectedProductMol = extractProducts(molReaction);
        ...
        for generatedProductMol1 in generatedProductIS1.GetOEGraphMols():
            ...
            generatedProductIS2 = virtual_oemolistream(productOEOS2.GetString(), OEFormat_ISM);
            ...
            if compareProducts( expectedProductMol, generatedProductIS2.GetOEGraphMols(),reactantMolList2)==True:
                isMatched=True;
        return isMatched;
            
    def runByThreeReactionPatterns( self, molReaction, molPattern1,molPattern2,molPattern3):
        """
        ...
        for generatedProductMol1 in generatedProductIS1.GetOEGraphMols():
            ...
            reactantMolList2 = extractReactants(molReaction2);
            generatedProductOEOS2 = generateProducts(molPattern2,reactantMolList2);
            generatedProductIS2 = virtual_oemolistream(generatedProductOEOS2.GetString(), OEFormat_ISM);
            ...
            for generatedProductMol2 in generatedProductIS2.GetOEGraphMols():
                ...
                generatedProductIS3 = virtual_oemolistream(generatedProductOEOS3.GetString(), OEFormat_ISM);
                ...
                if compareProducts( expectedProductMol, generatedProductIS3.GetOEGraphMols(),reactantMolList3 )==True:
                    isMatched=True;
        return isMatched;
    ...
    def runByReagent( self, reactionSmi, reagent ):
        ...
        try:
            ...
            matchFound = compareProducts( expectedProductMol, generatedProductMolList,reactantMolList);
...
def extractReactants(molReaction):
    ...
    reactantsMol.SetRxn(False); # Treat as "regular" composite molecule now, not a reaction
    return reactantsMol;
...
def extractProducts(molReaction):
    ...
    for atom in productsMol.GetAtoms():
        if atom.GetRxnRole() != OERxnRole_Product:
            ...
            atom.SetRxnRole(OERxnRole_None);
    productsMol.SetRxn(False); # Treat as "regular" composite molecule now, not a reaction
            ...
            for canonizedGeneratedSmiIndividual in canonizedGeneratedSmi.split("."):
                ...
                for canonizedExpectedSmiIndividual in canonizedExpectedSmi.split("."):
                    ...
                    #and compare with individual extracted product molecule
                        ...
                        if inReactantsMolList(reactantsMol,canonizedGeneratedSmiIndividual)== False:
                            if canonizedGeneratedSmiIndividual == canonizedExpectedSmiIndividual:
...
def reactionDatabaseistream(reactionDatabaseFile):
    ...
    if lowerreactionDatabaseFile.endswith(".rdf") or lowerreactionDatabaseFile.endswith(".rdf.gz"):
