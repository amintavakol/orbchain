            ...
            for reagent in self.reagents:
                ...
                if len(reactantPair) > 1:
                    ...
                    if reagent["expected_reactants"] > 1:
                        productList = reagent(
                            [
                                molBySmiles(reactantPair[0]),
                                molBySmiles(reactantPair[1]),
                            ],
                            supplementalData,
                        )
                else:
                    productList = reagent(
                        [molBySmiles(reactantPair[0])], supplementalData
                    )
                ...
                for products in productList:
                    ...
                    for product in products:
                        ...
                        self.productDetails.setdefault(
                            product,
                            (
                                {"Reactants": reactantPair},
                                {"ReagentID": reagent["reagent_id"]},
                            ),
                        )
