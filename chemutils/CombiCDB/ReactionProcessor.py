...
def main(argv):
    ...
    usageStr = (
        "usage: %prog [options] <reactantFile> <smirksFile> <productFile>\n"
    ...
    parser.add_option(
        ...
        "--incReactants",
        action="store_true",
        help="When generating products, set this flag if you wish to have the reactants listed along side the products, for easy depiction / viewing of the whole reaction.",
    )
    parser.add_option(
        "-s",
        "--ignoreSelfReactions",
        action="store_true",
        help="Don't count products generated from a reactant reacting with another copy of itself",
    )
    parser.add_option(
        "-d",
        "--DBPrepare",
    ...
    parser.add_option(
        ...
        dest="productFile",
        metavar="<productFile>",
        help="For use with -d option.  The product file created by a normal run of this module.  The label generated for each product molecule includes the information needed by the database preparation script.",
    )
    ...
    if options.DBPrepare != None and options.productFile != None and len(args) >= 3:
        instance.formatDBFileByFilename(
            options.productFile, args[0], args[1], args[2], options.DBPrepare
        )
    elif len(args) >= 3:
...
class ReactionProcessor:
    ...
    def generateProductsByFilename(
        self, reactantFilename, smirksFilename, productFilename
    ):
    ...
    def generateProducts(self, reactantOEISFactory, smirksFile, productOEOS):
        ...
        for iSmirks, smirks in enumerate(smirksList):
            ...
            nProducts = self.applyReaction(
                libgen, reactantOEISFactory, productOEOS, iSmirks
            )
    ...
    def applyReaction(
        ...
        if reactantList is None:
            reactantList = []
            # List of reactants added so far.
        ...
        if reactantPosition < libgen.NumReactants():
            ...
            if reactantPosition == 0:
                progressDots = ProgressDots(
                    EST_INPUT / 10, EST_INPUT / 200, "1st position reactants"
                )
            ...
            for iReactant, reactant in enumerate(reactantOEISIter):
                if not self.ignoreSelfReactions or iReactant not in currReactantIndexes:
                    reactantFits = libgen.SetStartingMaterial(
                        reactant, reactantPosition, False
                    )
                    # log.debug("%d%d%d",reactantPosition,iReactant,reactantFits)
                    if progressDots != None:
                        progressDots.Update()
                    if reactantFits:
                        # Adjust parameters for next recursion level
                        reactantList.append(reactant)
                        currReactantIndexes.append(
                            iReactant
                        )  # Set last index in list to that of current reactant

                        productCount += self.applyReaction(
                            libgen,
                            reactantOEISFactory,
                            productOEOS,
                            reactionIndex,
            ...
            productLabel = "%s%d%s%s%s" % (
                REACTION_LABEL,
                reactionIndex,
                REACTION_LABEL_END,
                REACTANT_LIST_LABEL,
                str(currReactantIndexes).replace(" ", ""),
            )
            ...
            for product in libgen.GetProducts():
                ...
                validProduct = self.productPostProcessing(
                    product, reactantOEISFactory, reactantList
                )
                if validProduct:
                    ...
                    rejectSmi = createStandardSmiString(product)
                    # Don't just use OECreateIsoSmiString,
                    rejectProductSmiSet.add(rejectSmi)
                    #   sometimes that has parsing errors on modified molecules
    ...
    def productPostProcessing(self, product, reactantOEISFactory, reactantList):
        ...
        for bond in product.GetBonds():
            ...
            if bond.GetOrder() == 2 and bond.IsInRing():
                if (
                    bond.GetBgn().GetDegree() == 2 + 1
                    and bond.GetEnd().GetDegree() == 2 + 1
                ):
                    if isBridgehead(bond.GetBgn(), ringSystemInfo) or isBridgehead(
                        bond.GetEnd(), ringSystemInfo
                    ):
                ...
                if bond.HasStereoSpecified():
                    ...
                    for atom in bond.GetBgn().GetAtoms():
                        if (
                            atom.GetIdx() != bond.GetEnd().GetIdx()
                            and atomRingMap[atom.GetIdx()] == bondRingIdx
                        ):
                            neighbors.append(atom)
                            break
                    for atom in bond.GetEnd().GetAtoms():
                        if (
                            atom.GetIdx() != bond.GetBgn().GetIdx()
                            and atomRingMap[atom.GetIdx()] == bondRingIdx
                        ):
                            neighbors.append(atom)
                            break
                    if (
                        bond.GetStereo(neighbors, OEBondStereo_CisTrans)
                        == OEBondStereo_Trans
                    ):
    ...
    def formatDBFileByFilename(
        ...
        try:
            self.formatDBFile(
                productFile, reactantIDFile, smirksIDFile, productIDFile, dbFile
            )
        finally:
            productFile.close()
    ...
    def formatDBFile(
        self, productOEIS, reactantIDFile, smirksIDFile, productIDFile, dbFile
    ):
        ...
        for iProduct, mol in enumerate(productOEIS.GetOEGraphMols()):
            label = mol.GetTitle().split()[
                0
            ]  # Take first token as functional label.  Dump any extra items
            # Parse it out, assuming a label structure like Smirks[x]Reactants[a,b,c,etc.]
            iSmirks, iReactantList = label.split(REACTION_LABEL)[1].split(
                REACTION_LABEL_END + REACTANT_LIST_LABEL
            )  # Remove labels
            iSmirks = int(iSmirks)
            iReactantList = (
                iReactantList[1:].split("]")[0].split(",")
            )  # Clip off brackets and split by commas
            ...
            for reactantPosition, iReactant in enumerate(iReactantList):
                dbFile.write(
                    "%s\t%s\t%s\t%d\n"
                    % (
                        productIDList[iProduct],
                        smirksIDList[iSmirks],
                        reactantIDList[iReactant],
                        reactantPosition,
                    )
                )
