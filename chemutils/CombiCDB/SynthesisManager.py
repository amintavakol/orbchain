...
from chemutils.Common.Util import ProgressDots, molBySmiles;
...
from chemutils.CombiCDB.Const import MAX_TRIES, CACHE_CODE, DISABLED_CODE, SAMPLE_REACTION_CLASS;
...
class SynthesisManager:
    ...
    def loadReactionSynthesisIDs( self, synthesisRequest, cacheOnly=False ):
        ...
        if len(synthesisRequest.reactionSynthesisIDs) > 0:
            query.addWhereIn("synth.reaction_synthesis_id", synthesisRequest.reactionSynthesisIDs );
        ...
        if synthesisRequest.exampleReagentId is not None:
            ...
            query.addWhere("generation_date is null");  # Do not include auto-generated reactions in demonstration examples

        if synthesisRequest.reactionStepId is not None or \
           synthesisRequest.reagentId is not None or \
           synthesisRequest.reactantSmi is not None:
            ...
            if synthesisRequest.reactionStepId is not None:
                query.addWhereEqual("step.reaction_step_id", synthesisRequest.reactionStepId );
        ...
        if len(synthesisRequest.reactionCategoryIDs) > 0:
            if not synthesisRequest.allowCategorySubsets and not synthesisRequest.allowCategorySupersets:
                # Subsets not allowed, only an exact match will be accepted
                query.addWhereEqual("reaction_category_ids", synthesisRequest.reactionCategoryIDsStr() );
            else:
                ...
                idStrList = [synthesisRequest.reactionCategoryIDsStr()];    # Start with exact matches to ensure a non-empty list
                for idSetStr, idSet in databaseIdSetData.iteritems():
                    if (synthesisRequest.allowCategorySubsets and idSet.issubset(requestIdSet) ) or \
                       (synthesisRequest.allowCategorySupersets and idSet.issuperset(requestIdSet) ):
    ...
    def loadReactionSyntheses( self, synthesisRequest, cacheOnly=False ):
        ...
        """
        reactionSynthesisIDs = self.loadReactionSynthesisIDs( synthesisRequest, cacheOnly );
        if len(reactionSynthesisIDs) < 1:
            return [];  # No IDs to search, nothing to return
        ...
        query.addOrderBy("reaction_synthesis_id");  # Must order by this first for subsequent loop to work correctly
        query.addOrderBy("position");
        query.addOrderBy("reaction_step_id");   # Ensure a consistent ordering
        if synthesisRequest.exampleReagentId is not None:
            query.addWhereEqual("reagent_id", synthesisRequest.exampleReagentId);

        reactionStepTable = DBUtil.execute( query, includeColumnNames=True, connFactory=self.connFactory );
        ...
        for iStep, reactionStepModel in enumerate(reactionStepModels):
            ...
            for iReactant, reactant in enumerate(reactants):
                ...
                reactantMol.SetIntData("reaction_step_id", reactionStepModel["reaction_step_id"]);
                reactantMol.SetIntData("reaction_synthesis_id", reactionStepModel["reaction_synthesis_id"]);
            ...
            if reactionStepModel["warning_level_ids"] is not None and len(reactionStepModel["warning_level_ids"]) > 0:
                warning_level_ids = reactionStepModel["warning_level_ids"].split(REACTION_PROFILE_DELIM);
                warning_messages = reactionStepModel["warning_messages"].split(REACTION_PROFILE_DELIM);

                for product, warning_level_id, warning_message in zip(products, warning_level_ids, warning_messages):
        ...
        if synthesisRequest.userID is not None:
            ...
            for preCachedSynthesis in synthesisList:
                ...
                for productMol in reactionSteps[-1][PRODUCTS]:   # Check final product list for products already seen / solved for
                    productSmiles = createStandardSmiString(productMol);
                    productSeenBefore = productSeenBefore or productSmiles in productSmilesSeenBeforeSet;
    ...
    def saveReactionSynthesis( self, reactionSteps, synthesisRequest=None, code=None, problemClass=None, conn=None ):
        ...
        try:
            ...
            reaction_synthesis_id = DBUtil.execute( DBUtil.identityQuery("reaction_synthesis"), conn=conn )[0][0];

            for iStep, reactionStep in enumerate(reactionSteps):
                reactant_smiles = str.join(MOL_LIST_DELIM, molToSmiList(reactionStep[REACTANTS]));
                reagent_id = reactionStep[REAGENT]["reagent_id"];
                product_smiles = str.join(MOL_LIST_DELIM, molToSmiList(reactionStep[PRODUCTS]));

                warning_level_ids = [];
                warning_messages = [];
                for product in reactionStep[PRODUCTS]:
                    warning_level_ids.append(str(product.GetIntData("warning_level_id")));
                ...
                reaction_step_id = DBUtil.execute( DBUtil.identityQuery("reaction_step"), conn=conn )[0][0];

                # Reconstruct and store reaction_step_reaction_profile records
                reactionProfileIdSet = self.assignStepReactionProfileIds(reactant_smiles, reagent_id, conn=conn);
                self.updateReactionStepReactionProfile(reaction_step_id, reactionProfileIdSet, conn=conn);
    ...
    def deleteReactionSynthesis( self, clearIDs, conn=None, softDelete=False ):
        ...
        try:
            ...
            clearIDs = tuple(clearIDs); # So it will work with DB parameter replacement
            ...
            if not softDelete:
                ...
                query = """DELETE FROM reaction_step WHERE reaction_synthesis_id IN (%s)""" % placeholders;
                cursor.execute( query, clearIDs );

                query = """DELETE FROM reaction_synthesis WHERE reaction_synthesis_id IN (%s)""" % placeholders;
                cursor.execute( query, clearIDs );
            else:
                # Just "soft delete" the problems by relabeling them
                query = """UPDATE reaction_synthesis SET code = '%s' WHERE reaction_synthesis_id IN (%s)""" % (DISABLED_CODE, placeholders);
                cursor.execute( query, clearIDs );

            conn.commit();  # Only commit after both deletes were completed together as a single transaction                
        finally:
            cursor.close();
            conn.close();
        ...
        try:
            ...
            for reactionSteps in synthesisList:
                if len(reactionSteps) > 0 and len(reactionSteps[0][REACTANTS]) > 0:
                    oldSynthesisId = reactionSteps[0][REACTANTS][0].GetIntData("reaction_synthesis_id");
                    oldSynthesisModel = modelsById[oldSynthesisId];

                    # Clone a copy of the detailed reaction steps
                    newSynthesisId = self.saveReactionSynthesis( reactionSteps, conn=conn );

                    # Prepare a revised version of the problem data code / title for the clone
                    newSynthesisModel = dict(oldSynthesisModel);
                    del newSynthesisModel["reaction_synthesis_id"]; # Keep the new ID
                    del newSynthesisModel["generation_date"];   # Keep the new date
                    newSynthesisModel["code"] = "%(code)s (CLONED from #%(reaction_synthesis_id)s)" % oldSynthesisModel;

                    # Update the the clone with respective descriptive metadata
                    DBUtil.updateRow("reaction_synthesis", newSynthesisModel, newSynthesisId, conn=conn);
    ...
    def moveStepPosition( self, reactionStepId, direction ):
        ...
        connFactory = self.connFactory;
        if connFactory is None:
            connFactory = DBUtil.ConnectionFactory();   # Use default connection source
            ...
            for iStep, (stepId, position) in enumerate(stepDataTable):
                if stepId == reactionStepId:
                    ...
                    if stepIdA is not None:
                        ...
                        updateQuery = """update reaction_step set position = %(p)s where reaction_step_id = %(p)s""" % {"p": DBUtil.SQL_PLACEHOLDER};
                        DBUtil.execute( updateQuery, (positionB, stepIdA), conn=conn );
                        DBUtil.execute( updateQuery, (positionA, stepIdB), conn=conn );
                        conn.commit();

                    break;  # Do not need to look further
        finally:
            conn.close();
    ...
    def reassignSteps( self, reactionStepIdList, reassignListId ):
        ...
        try:
            ...
            completedStepIds = set();   # Keep track of completion to avoid repeats
            for reactionStepId in reactionStepIdList:
                if reactionStepId not in completedStepIds:
                    currentMaxPosition += 10;   # Arbitrary spacer number to expand out position values
                    DBUtil.execute( updateQuery, (reassignListId, currentMaxPosition, reactionStepId), conn=conn );
    ...
    def minimizeSynthesisId( self, reaction_synthesis_id ):
        ...
        conn = connFactory.connection();
        try:
            minAvailableId = DBUtil.smallestAvailableValue("reaction_synthesis", "reaction_synthesis_id", conn=conn);
    ...
    def minimizeStepId( self, reaction_step_id, conn ):
        ...
        """
        minAvailableId = DBUtil.smallestAvailableValue("reaction_step", "reaction_step_id", conn=conn);
        ...
        if len(linkProfileIds) > 0:
            ...
            placeholderId = -1;
            placeholderModel = {"reaction_step_id": placeholderId, "reaction_synthesis_id": placeholderId};
    ...
    def assignStepReactionProfileIds(self, reactantSmiles, reagentId, conn=None):
        ...
        reactants = [molBySmiles(smi) for smi in reactantSmiles.split(MOL_LIST_DELIM)];  # List of molecule objects as reactant input
        ...
        reagentManager.returnReagentsById( reagentsById ); # Return used reagent objects to cache for later reuse
        
        return reactionProfileIdSet;
    
    def assignSynthesisReactionProfileIds(self, reactionSynthesisId, checkExisting=True, conn=None):
        ...
        if checkExisting:
            ...
            grandChildQuery = \
                ...
                order by
                    ...
                    rsrp.reaction_profile_id
                """ % DBUtil.SQL_PLACEHOLDER;
            linkedData = DBUtil.execute( grandChildQuery, (reactionSynthesisId,), conn=conn, connFactory=self.connFactory);
        ...
        if len(reactionProfileIdSetList) < 1:
            ...
            for (stepId, reagentId, reactantSmiles) in reactionStepData:
                stepReactionProfileIds = self.assignStepReactionProfileIds(reactantSmiles, reagentId, conn);
                reactionProfileIdSetList.append(stepReactionProfileIds);
            
        return reactionProfileIdSetList;
    
    def updateReactionSynthesisReactionProfile(self, reactionSynthesisId, reactionProfileIdSetList, conn=None):
        ...
        for (stepId, reactionProfileIdSet) in zip(reactionStepIdList, reactionProfileIdSetList):
            self.updateReactionStepReactionProfile( stepId, reactionProfileIdSet, conn=conn)
        
        # Commit changes to database so we don't leave too much hanging in the buffer
        if conn is not None:
            conn.commit();
    
    def updateReactionStepReactionProfile(self, reactionStepId, reactionProfileIdSet, conn=None):
    ...
    def clearProblemCacheByReactants( self, reactionCategoryId, reactantSmiSet ):
        ...
        synthesisRequest.allowCategorySupersets = True;  # Any problem which includes this category is suspect
        ...
        for reactantSmi in reactantSmiSet:
            ...
            deleteIds.update( self.loadReactionSynthesisIDs(synthesisRequest, cacheOnly=True) );
