...
from chemutils.Common.Util import stdOpen
...
from chemutils.score.orbital.TransitionStateEnergyScore import (
...
class ReactionSimulator:
    ...
    def runSimulation(self, reactionMixture, depth=0, reactionProposerParams=None):
        ...
        if self.transitionEnergyCalc is None:
            self.transitionEnergyCalc = TransitionStateEnergy.getInstance()
            # Should propagate reactionMixture.temperature parameter?
        ...
        for filledOrb, unfilledOrb in reactionProposer(reactionMixture):
            ...
            if reactionSmiles not in reactionMixture.elementaryStepHistory:
                elementaryStep = self.analyzeOrbitalPair(
                    filledOrb, unfilledOrb, reactionMixture
                )
                if elementaryStep.isProductive():
                    reactionMixture.elementaryStepHistory[
                        reactionSmiles
                    ] = elementaryStep
        ...
        reactionMixture = self.applyElementaryStepsToReactionMixture(
            reactionMixture, reactionMixture.elementaryStepHistory
        )
        ...
        if not stoppingCriteriaMet:
            self.runSimulation(
                reactionMixture,
                depth + 1,
                reactionProposerParams=reactionProposerParams,
            )
    ...
    def analyzeOrbitalPair(self, filledOrb, unfilledOrb, reactionMixture):
        ...
        arrowObjList = []
        reactionSmiles = reactionSmilesFromOrbitalPair(
            filledOrb, unfilledOrb, arrowObjList=arrowObjList
        )
        ...
        elementaryStep[
            "cationSolvationPotential"
        ] = reactionMixture.solventModel.cationSolvationPotential
        elementaryStep[
            "anionSolvationPotential"
        ] = reactionMixture.solventModel.anionSolvationPotential
    ...
    def applyElementaryStepsToReactionMixture(
        self, reactionMixture, elementaryStepHistory
    ):
        ...
        simulationScale = self.calculateSimulationScale(
            reactionMixture, elementaryStepHistory
        )
        ...
        for reactionSmiles, elementaryStep in elementaryStepHistory.iteritems():
            ...
            simulationDelta = self.calculateSimulationDelta(
                elementaryStep, reactionMixture, simulationScale
            )
        ...
        for reactionSmiles, simulationDelta in simDeltaByReactionSmiles.iteritems():
            ...
            if simulationDelta != 0:
                for reactantSmi in elementaryStep["reactantSmilesList"]:
                    ...
                    reactionMixture[reactantSmi].updateQuantity(
                        currentQuantity - simulationDelta
                    )
                ...
                for productSmi in elementaryStep["productSmilesList"]:
                    ...
                    reactionMixture[productSmi].updateQuantity(
                        currentQuantity + simulationDelta
                    )
    ...
    def calculateSimulationScale(self, reactionMixture, elementaryStepHistory):
        ...
        if limitingComponentSmi is not None:
            ...
            if (
                limitQuantity * SIMULATION_ITERATION_MAX_FRACTION
            ) < SIMULATION_ITERATION_MIN_UNIT:
                ...
                minimumQuantity = float(
                    min(limitQuantity, SIMULATION_ITERATION_MIN_UNIT)
                )
                ...
                simulationScale = (
                    SIMULATION_ITERATION_MAX_FRACTION * limitQuantity / limitRate
                )
    ...
    def calculateSimulationDelta(
        self, elementaryStep, reactionMixture, simulationScale=1.0
    ):
...
class ReactionMixture(dict):
    ...
    def __init__(self, initReactionMixture=None):
        ...
        # Keep track of how much simulation history has been recorded
        self.scaleHistory = []
        # Keep track of relative step size of each change history
    ...
    def addMoleculeQuantity(self, smiles, quantity, pastHistory=None):
        ...
        if pastHistory is not None:
            ...
            if len(pastHistory) != self.currentIteration:
                raise Exception(
                    "Expected %d elements in reaction quantity history but instead found %s"
                    % (self.currentIteration, pastHistory)
                )
    ...
    def parseScaleHistory(self, historyStr):
        ...
        if len(self.scaleHistory) != self.currentIteration:
            raise Exception(
                "Expected %d elements in reaction simulation scaling history but instead found %s"
                % (self.currentIteration, self.scaleHistory)
            )
...
class ReactionMixtureComponent(dict):
    ...
    def pastHistoryStr(self):
        ...
        """
        pastHistoryStrList = []
        for value in self["quantityHistory"][
            :-1
        ]:  # Skip the current value as not being a *past* value
            pastHistoryStrList.append(str(value))
        historyStr = str.join(VALUE_DELIM, pastHistoryStrList)
        # Past values
        return historyStr
...
def main(argv):
    ...
    usageStr = (
        "usage: %prog [options] <reactantFile> <productFile>\n"
