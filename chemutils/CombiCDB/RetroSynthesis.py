...
from chemutils.Common.Util import ProgressDots, molBySmiles, generateFingerprint, stdOpen;
...
from chemutils.CombiCDB.ReactionModel import SMIRKSReagent, SupplementalDataModel;
from chemutils.CombiCDB.ReactionModel import REACTANTS, REAGENT, PRODUCTS, reactionStepStr;
...
from chemutils.CombiCDB.SynthesisUtil import GreedyRetroProductSelector, ExhaustiveRetroProductSelector;

from chemutils.CombiCDB.Const import MAX_TRIES, SIMILARITY_THRESHOLD;
...
class RetroSynthesis:
    ...
    def __init__(self, reagentList, reactantPool, retroProductSelector):
        ...
        for reagent in reagentList:
            ...
            retroReagent.countIntermediates = True; # Reactant may have already been half-processed (e.g., aldehyde, not alcohol, oxidized to carboxylate)
            retroReagent.removeInherentProducts = True; # Not interested in these
    ...
    def __call__(self, targetMol, depth=0):
        ...
        if targetMol in self.reactantPool:
            return ([], True);  # Base case, return blank synthesis pathway, target is already in the reactant pool
        ...
        retroProductsByReagent = self.retroProductSelector( self.retroReagentList, targetMol );
        ...
        for (retroProduct, retroReagent, iReagent) in retroProductsByReagent:
            ...
            for componentProductSmi in componentProductSmiList:
                ...
                (precursorSteps, precursorExactMatch) = self( componentMol, depth+1 ); # Recursive call
            ...
            reactionSteps.append( [ componentProductSmiList, self.reagentList[iReagent], [targetSmi] ] );
    ...
    def main(argv):
        ...
        usageStr = \
            """usage: %prog [options] <productSmiles>
        ...
        parser.add_option("-d", "--maxDepth",       dest="maxDepth",        metavar="<maxDepth>",       default="4",    help="Limit the retro-synthesis search depth to only this many reaction steps.")
        parser.add_option("-s", "--synthSteps",     dest="synthSteps",      metavar="<synthSteps>",     default="4",    help="If no product is specified, will auto-generate a random one with the SynthesisGenerator, using up to this many reaction steps")
        parser.add_option("-i", "--iterations",     dest="iterations",      metavar="<iterations>",     default="1",    help="If no product is specified, will test by auto-generating this many random products.");
        parser.add_option("-C", "--ChemDB",         dest="ChemDB",          action="store_true",        help="If set, will use the entire linked ChemDB as the available reactant pool instead of just the sample test reactant pool.")
        parser.add_option("-g", "--greedyChoices",  dest="greedyChoices",   metavar="<greedyChoices>",  default="1",    help="When doing a greedy search, number of paths to consider at any branch point.  Set to 0 to do an exhaustive search.");
        parser.add_option("-o", "--outputFile",     dest="outputFile",      metavar="<outputFile>",     default="-",    help="Name of file to output results to.  Defaults to stdout");
        ...
        if len(args) > 0:   origProductSmiles = args[0];
        ...
        if greedyChoices > 0:
            productSelector = GreedyRetroProductSelector( reactantPool, AverageScoreAggregator(), greedyChoices );
        else:
...
class ReactionPlanner:
    ...
    def __call__(self, startingMaterialList, reagentList, targetMol, maxSteps):
        ...
        """
        return self.addReactionSteps( startingMaterialList, reagentList, targetMol, maxSteps );

    def addReactionSteps( self, reactantList, reagentList, targetMol, maxSteps, reactionSteps=None, usedReactantSmiList=None ):
        ...
        if len(reactionSteps) < 1:
            ...
            keyReactantList.extend(SynthesisGenerator.selectBestProducts(carryoverProductList));
        ...
        for keyReactant in keyReactantList:
            ...
            for reagent in reagentList:
                for matchSteps in self.applyReagentStep(currentReactants, reagent, reactantList, reagentList, targetMol, maxSteps, reactionSteps, usedReactantSmiList ):
                    yield matchSteps;
    ...
    def applyReagentStep(self, currentReactants, reagent, reactantList, reagentList, targetMol, maxSteps, reactionSteps, usedReactantSmiList ):
        ...
        if len(currentProducts) > 0:
            acceptableStep = self.synthesisGenerator.acceptableReactionStep(currentReactants, reagent, currentProducts, supplementalData, self.reactionCategoryIdSet );
            ...
            if acceptableStep:
                ...
                for subMatch in self.addReactionSteps( reactantList, reagentList, targetMol, maxSteps, reactionSteps, usedReactantSmiList ):
                    yield subMatch;
