...
# TODO -- set logging level appropriately for production/testing
# import logging
...
class ArrowConversionError(Exception):
    ...
    def __init__(self, msg, mol=None, arrowList=None, orbTupleList=None):
        ...
        # Always have the msg, everything else is optional
        self.msg = msg
        ...
        if self.arrowList is not None:
            self.extraInfoStr = "Arrows: %s" % ElectronArrow.prepareArrowCodes(
                arrowList
            )
            ...
            self.extraInfoStr = "orbTupleList : %s" % pformat(
                [
                    [orb.toLabeledSmiAndInfoStr() for orb in theTuple]
                    for theTuple in self.orbTupleList
                ]
            )
        self.smi = ""
        try:
    ...
    def __str__(self):
        return """ArrowConversionError - %s - %s - %s""" % (
            self.smi,
            self.extraInfoStr,
            self.msg,
        )
...
def orbitalPairFromArrowList(mol, arrowList):
    ...
    log.debug(
        "After Canonical the arrowList is %s "
        % str(ElectronArrow.prepareArrowCodes(arrowList))
    )
...
def orbitalPairFromRadicalElectronArrowList(mol, arrowList):
    ...
    if len(arrowList) == 2:
        # homolytic bond dis
        if (
            arrowList[0].isBondDissociationArrow()
            and arrowList[1].isBondDissociationArrow()
        ):
        ...
        # dimerization
            ...
            sourceOrb = Orbital(
                arrowList[0].sourceAtoms[0], None, None, electrons=1, mol=mol
            )
            targetOrb = Orbital(
                arrowList[1].sourceAtoms[0], None, None, electrons=1, mol=mol
            )
        else:
            raise ArrowConversionError(
                mol=mol, arrowList=arrowList, msg="Ambigous radical arrows."
            )
        ...
        raise ArrowConversionError(
            mol=mol, arrowList=arrowList, msg="Not enough radical arrows"
        )
...
def orderRadicalElectronArrowList(mol, arrowList):
    ...
    if len(bondFormArrowList) + len(bondDisArrowList) != 2:
        raise ArrowConversionError(
            mol=mol,
            arrowList=arrowList,
            msg="Ambiguous free radical arrows involving bond pivots!",
        )
    ...
    while pivotArrowList != [] and foundOneBack:
        ...
        for i, pivotArr in enumerate(pivotArrowList):
            ...
            if (
                pivotArr.sourceIndexes[0] == currYAtom
                or pivotArr.targetIndexes[1] == currYAtom
            ):
    ...
    if pivotArrowList != []:
        raise ArrowConversionError(
            mol=mol,
            arrowList=arrowList,
            msg="Unable to chain radical bond pivot arrows. %s, %s"
            % (
                str([str(p) for p in pivotArrowList]),
                str([str(p) for p in orderedArrowList]),
            ),
        )
    ...
    if len(bondDisArrowList) == 1:
        ...
        if (
            orderedArrowList[0].sourceIndexes[1] != bondFormArrow.targetIndexes[0]
            or orderedArrowList[-1].sourceIndexes[1] != bondDisArrow.sourceIndexes[0]
        ):
            # This could be b/c the ordered list is flipped.
            orderedArrowList.reverse()
            if (
                orderedArrowList[0].sourceIndexes[1] != bondFormArrow.targetIndexes[0]
                or orderedArrowList[-1].sourceIndexes[1]
                != bondDisArrow.sourceIndexes[0]
            ):
                raise ArrowConversionError(
                    mol=mol,
                    arrowList=arrowList,
                    msg="Non-Matching dissociation and formation radical arrows!",
                )
        ...
        if bondDisArrow.sourceIndexes[1] == orderedArrowList[0].sourceIndexes[0]:
            ...
            if bondDisArrow.sourceIndexes[0] == orderedArrowList[-1].sourceIndexes[1]:
                ...
                raise ArrowConversionError(
                    mol=mol,
                    arrowList=arrowList,
                    msg="Ambiguous radical bond dissociation arrow",
                )
        ...
        if bondFormArrow.targetIndexes[0] == orderedArrowList[0].sourceIndexes[1]:
            ...
            if bondFormArrow.sourceIndexes[0] == orderedArrowList[-1].targetIndexes[1]:
                ...
                raise ArrowConversionError(
                    mol=mol,
                    arrowList=arrowList,
                    msg="Ambiguous radical bond form arrow",
                )
            ...
            raise ArrowConversionError(
                mol=mol, arrowList=arrowList, msg="Ambiguous radical bond form arrow"
            )
...
def orbitalPairFromTwoElectronArrowList(mol, arrowList):
    ...
    log.debug(
        "Before start the arrowList is %s "
        % str(ElectronArrow.prepareArrowCodes(arrowList))
    )
    ...
    while arrowList != []:
        ...
        if len(sourceIdx) == 1 and len(targetIdx) == 1:
            ...
            potIdx = [
                ...
                if len(arrowList[i].sourceIndexes) == 2
                and len(arrowList[i].targetIndexes) == 1
                and arrowList[i].sourceIndexes[0] == targetIdx[0]
                and arrowList[i].sourceIndexes[1] == arrowList[i].targetIndexes[0]
            ]
            log.debug("Got these for potIdx %s" % str(potIdx))
            if len(potIdx) > 1:
                raise ArrowConversionError(
                    mol=mol, arrowList=arrowList, msg="Ambiguous arrow codes!!"
                )
            ...
            if len(potIdx) == 1:
                ...
                targetOrb = bondOrbitalByMapIdx(
                    mol, targetIdx[0], extArrow.sourceIndexes[1]
                )

            else:
                # orb(a2)
                log.debug(
                    "No matching a2,a3=a3 patterns, just the singleton arrow movement.  Make non-bonded target orb"
                )
            ...
            if sourceIdx[1] == targetIdx[0] and len(targetIdx) == 1:
                log.debug(
                    "have a1,a2=a2 pattern!  Look for a3=a1 or a3,a4=a4,a1 or nothing"
                )
                ...
                potIdx = [
                    i
                    for i in xrange(len(arrowList))
                    if arrowList[i].targetIndexes[0] == sourceIdx[0]
                    and len(arrowList[i].targetIndexes) == 1
                ]
                ...
                if len(potIdx) > 1:
                    ...
                    potIdx = [
                        i
                        for i in xrange(len(arrowList))
                        if len(arrowList[i].targetIndexes) == 2
                        and arrowList[i].targetIndexes[1] == sourceIdx[0]
                        and arrowList[i].sourceIndexes[1]
                        == arrowList[i].targetIndexes[0]
                    ]
                    ...
                    if len(potIdx) > 1:
                        ...
                        extArrow = arrowList.pop(potIdx[0])
                        sourceOrb = bondOrbitalByMapIdx(
                            mol, extArrow.sourceIndexes[1], extArrow.sourceIndexes[0]
                        )
                ...
                potIdx = [
                    i
                    for i in xrange(len(arrowList))
                    if arrowList[i].sourceIndexes[0] == targetIdx[1]
                    and len(arrowList[i].sourceIndexes) == 2
                    and len(arrowList[i].targetIndexes) == 1
                    and arrowList[i].sourceIndexes[1] == arrowList[i].targetIndexes[0]
                ]
                ...
                if len(potIdx) == 1:
                    ...
                    targetOrb = bondOrbitalByMapIdx(
                        mol, extArrow.sourceIndexes[0], extArrow.targetIndexes[0]
                    )
                else:
    ...
    log.debug(
        "The curr orbTupleList is : %s"
        % str([(str(pair[0]), str(pair[1])) for pair in orbTupleList])
    )
...
def collapseOrbitalPairs(orbTupleList, numElectrons=2):
    ...
    if len(potIdx) == 0:
        log.warning(
            "Cant find a good break point, just using first tuple.  orbTupleList: %s"
            % str(
                [
                    [orb.toLabeledSmiAndInfoStr() for orb in theTuple]
                    for theTuple in orbTupleList
                ]
            )
        )
        potIdx = [0]
    ...
    log.debug(
        "SourceBreak:%s, and TargetBreak:%s"
        % (str(breakOrbTuple[0]), str(breakOrbTuple[1]))
    )
    ...
    while orbTupleList != [] and contLookingForTargets:
        ...
        potIdx = [
            i
            for i in xrange(len(orbTupleList))
            if orbTupleList[i][0].neighbor is not None
            and orbTupleList[i][0].neighbor.GetMapIdx() == currTargetAtmIdx
        ]
        if len(potIdx) > 0:
        ...
        contLookingForTargets = (
            foundSomething and targetList[-1].type not in BOND_ORBITAL_TYPES
        )
    ...
    while orbTupleList != [] and contLookingForSources:
        ...
        potIdx = [
            i
            for i in xrange(len(orbTupleList))
            if orbTupleList[i][1].atom.GetMapIdx() == currSourceNeigIdx
            and orbTupleList[i][1].type not in BOND_ORBITAL_TYPES
        ]
        if len(potIdx) > 0:
        ...
        contLookingForSources = (
            foundSomething and sourceList[-1].type in BOND_ORBITAL_TYPES
        )
    ...
    if orbTupleList != []:
        ...
        raise ArrowConversionError(
            msg="Unable to chain the orbitals. Left over orbs. \n sourceOrbList : %s \n targetList: %s"
            % (
...
def canonicalOrbitalPair(sourceOrb, targetOrb, nElectrons=None):
    ...
    if len(sourceOrbList) == 1 and len(targetOrbList) == 1:
        ...
        if (
            targetOrb.isPerceivedFreeRadical()
            and sourceOrb.isBondOrbital()
            and not targetOrb.isBondOrbital()
        ):
            return targetOrb, sourceOrb
        # Dimerization, one with lowest mapIdx should be first
        if (
            targetOrb.isPerceivedFreeRadical()
            and sourceOrb.isPerceivedFreeRadical()
            and not sourceOrb.isBondOrbital()
            and not targetOrb.isBondOrbital()
        ):
    ...
    for i in xrange(1, len(sourceOrbList)):
    ...
    for i in xrange(1, len(targetOrbList)):
    ...
    if len(sortedKeys) < 1:
        log.warning(
            "IN Cononicalization, can't find breaks/forms in %s, %s"
            % (
                pformat(sourceOrb.toLabeledSmiAndInfoStr()),
                pformat(targetOrb.toLabeledSmiAndInfoStr()),
            )
        )
        return sourceOrb, targetOrb

    typeId, breakIdx = atmIdxDict[sortedKeys[0]]
    ...
    if (
        sourceOrbList[-1].neighbor is not None
        and targetOrbList[-1].type not in BOND_ORBITAL_TYPES
        and sourceOrbList[-1].neighbor.GetMapIdx() == targetOrbList[-1].atom.GetMapIdx()
    ):
