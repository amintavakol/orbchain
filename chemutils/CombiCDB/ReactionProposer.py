...
from chemutils.CombiCDB.OrbitalInteraction import ElementaryStepData, isBondDissociation, isPericyclic;
...
from chemutils.score.orbital.Util import log;
...
class BaseReactionProposer:
    ...
    def getOrbitalPairFilters( self ):
        ...
        if self.orbitalPairFilters is None:
            ...
            self.orbitalPairFilters = list();   # Default to empty list
    ...
    def __call__(self, reactionMixture, analyzedMoleculeSmiSet=None):
        ...
        """
        for (filledOrb, unfilledOrb) in self.proposeOrbitalPairs( reactionMixture, analyzedMoleculeSmiSet ):
    ...
    def passOrbitalPairFilters( self, filledOrb, unfilledOrb ):
        ...
        for orbitalPairFilter in self.getOrbitalPairFilters():
            ...
            if not orbitalPairFilter( filledOrb, unfilledOrb ):
                passFilters = False;
                break;  # Don't need to check anymore if one already failed
        return passFilters;
    ...
    def checkAnalysisHistory( reactionMixture ):
        ...
        for reactionSmiles, elementaryStepData in reactionMixture.elementaryStepHistory.iteritems():
    ...
    def isNewReactant(mol, analyzedMoleculeSmiSet):
        ...
        """
        molCopy = OEGraphMol(mol);   # Make a copy we're free to modify
...
class CompositeReactionProposer(BaseReactionProposer):
    ...
    def __init__(self, reactionProposerParams=None):
        ...
        self.reactionProposers = None;    # Start with empty list of possibilities
    ...
    def proposeOrbitalPairs( self, reactionMixture, analyzedMoleculeSmiSet=None ):
        ...
        (analyzedMoleculeSmiSet, newSpeciesFound) = self.checkAnalysisHistory( reactionMixture );

        if newSpeciesFound:
            for reactionProposer in self.reactionProposers:
                for orbitalPair in reactionProposer(reactionMixture, analyzedMoleculeSmiSet):
...
class BaseFilledUnfilledProposer(BaseReactionProposer):
    ...
    def getFilledOrbitalGenerator( self, reactionMixture, restrictSymmetry=True ):
        ...
        if self.filledOrbGenerator is None:
            ...
            if self.acceptAllOrbitals:
                filledOrbReactivityCalc = AcceptAllOrbitalReactivityProbability(True, reactionMixture);
            else:
                filledOrbReactivityCalc = OrbitalReactivityProbability(True, reactionMixture);
            self.filledOrbGenerator = ReactionMixtureOrbitalFactory( reactionMixture, filledOrbReactivityCalc, restrictSymmetry);
    ...
    def getUnfilledOrbitalGenerator( self, reactionMixture, restrictSymmetry=False ):
        ...
        if self.unfilledOrbGenerator is None:
            ...
            if self.acceptAllOrbitals:
                unfilledOrbReactivityCalc = AcceptAllOrbitalReactivityProbability(False, reactionMixture);
            else:
                unfilledOrbReactivityCalc = OrbitalReactivityProbability(False, reactionMixture);
            self.unfilledOrbGenerator = ReactionMixtureOrbitalFactory( reactionMixture, unfilledOrbReactivityCalc, restrictSymmetry);
...
class OrbitalPairCombinationProposer(BaseFilledUnfilledProposer):
    ...
    def proposeOrbitalPairs( self, reactionMixture, analyzedMoleculeSmiSet=None ):
        ...
        filledOrbGenerator = self.getFilledOrbitalGenerator(reactionMixture, restrictSymmetry=False);
        unfilledOrbGenerator = self.getUnfilledOrbitalGenerator(reactionMixture, restrictSymmetry=True);
        ...
        for unfilledOrb in unfilledOrbGenerator:
            unfilledOrb.dataTags["fromNewReactant"] = self.isNewReactant(unfilledOrb.mol, analyzedMoleculeSmiSet);
            
            unfilledOrbList.append( unfilledOrb );
            
        for filledOrb in filledOrbGenerator:
            filledOrb.dataTags["fromNewReactant"] = self.isNewReactant(filledOrb.mol, analyzedMoleculeSmiSet);

            for unfilledOrb in unfilledOrbList:
                isUnfilledOrbNew = self.isNewReactant(unfilledOrb.mol, analyzedMoleculeSmiSet);

                if not filledOrb.dataTags["fromNewReactant"] and not unfilledOrb.dataTags["fromNewReactant"]:
                ...
                if filledOrb.mol == unfilledOrb.mol:
                    ...
                    if compatibleOrbitalPair(filledOrb, unfilledOrb):
                        ...
                        if cloneFilledOrb.toLabeledSmiAndInfoStr() != cloneUnfilledOrb.toLabeledSmiAndInfoStr():
...
class LocalInteractionProposer(BaseFilledUnfilledProposer):
    ...
    def proposeOrbitalPairs( self, reactionMixture, analyzedMoleculeSmiSet=None ):
        ...
        # Prepare generators for all of the separate filled and unfilled orbitals we want to consider in combination
        filledOrbGenerator = self.getFilledOrbitalGenerator(reactionMixture, restrictSymmetry=True);
        unfilledOrbGenerator = self.getUnfilledOrbitalGenerator(reactionMixture, restrictSymmetry=True);
        ...
        for filledOrb in filledOrbGenerator:
            ...
            if filledOrb.dataTags["fromNewReactant"]:
                ...
                for (localFilledOrb, localUnfilledOrb) in self.prepareFilledLocalOrbitalPairs( filledOrb ):
        ...
        for unfilledOrb in unfilledOrbGenerator:
            ...
            if unfilledOrb.dataTags["fromNewReactant"]:
                # Check for local interactions with this orbital that may be missed in the main loop
                for (localFilledOrb, localUnfilledOrb) in self.prepareUnfilledLocalOrbitalPairs( unfilledOrb ):
                    yield (localFilledOrb, localUnfilledOrb);
    ...
    def prepareFilledLocalOrbitalPairs( self, filledOrb ):
        ...
        for neighbor in filledOrb.atom.GetAtoms():
            for neighborOrb in orbitalIter(neighbor):
                hasInteractionPotential = (not neighborOrb.isLonePair());   # Anything but a lone pair could be interpreted as an unfilled orbital
    ...
    def prepareUnfilledLocalOrbitalPairs( self, unfilledOrb ):
        ...
        for neighbor in unfilledOrb.atom.GetAtoms():
            for neighborOrb in orbitalIter(neighbor):
                hasInteractionPotential = (not neighborOrb.isEmpty());   # Anything but a an empty orbital could be interpreted as a filled orbital
...
class PericyclicUnimolecularProposer(BaseReactionProposer):
    ...
    def addPatternList(self, patternInfoTuples):
        ...
        for (priority, smartsPattern, sourceOrbInfoStr, targetOrbInfoStr) in patternInfoTuples:
            # Only want to initialize the pattern matchers once.  Relatively expensive process.
            matcher = OESubSearch();
            matcher.Init(smartsPattern);

            patternMatcherList.append( (priority, smartsPattern, matcher, sourceOrbInfoStr, targetOrbInfoStr) );
    ...
    def setupDefaultPatternMatchers(self):
        ...
        # 6 pi
        self.addPatternList \
        (   [   ( 100, "[*:1](/[*:10])([*:11])=[*:2]/[*:3]=[*:4]\[*:5]=[*:6]([*:60])(/[*:61])", "1 pi 2 2 2,10,11; 3 pi 4 2; 5 pi 6 2 None 5,60,61", "6 p None 0 5,60,61"),
                (   0, "[*:1]=[*:2][*:3]=[*:4][*:5]=[*:6]", "1 pi 2 2; 3 pi 4 2; 5 pi 6 2", "6 p None 0"),
            ]
        );
        ...
        # [3,3]
        self.addPatternList( [( 0, "[*:1]=[*:2][*:3]-[*:4][*:5]=[*:6]", "1 pi 2 2; 3 sigma 4 2; 5 pi 6 2", "6 p None 0")] );
    ...
    def proposeOrbitalPairs(self, reactionMixture, analyzedMoleculeSmiSet=None ):
        ...
        for smiles, mixtureComponent in reactionMixture.iteritems():
            ...
            if mixtureComponent.currentQuantity() > 0 and self.isNewReactant(mol, analyzedMoleculeSmiSet):
                ...
                for patternMatcherList in self.patternMatcherLists:
                    ...
                    for (priority, smartsPattern, matcher, sourceOrbInfoStr, targetOrbInfoStr) in patternMatcherList:
                        ...
                        for match in matcher.Match(mol,False):
                            ...
                            for atomMatchPair in match.GetAtoms():
                                atomMatchPair.target.SetMapIdx( atomMatchPair.pattern.GetMapIdx() );

                            sourceOrb = Orbital.fromLabeledMolAndInfoStr(mol, sourceOrbInfoStr);
                            targetOrb = Orbital.fromLabeledMolAndInfoStr(mol, targetOrbInfoStr);
                            ...
                            redundancyKey = self.orbitalInteractionRedundancyKey( sourceOrb, targetOrb );
                            
                            redundantPriority = None;
                            if redundancyKey in priorityByRedundancyKey:
                                redundantPriority = priorityByRedundancyKey[redundancyKey];
                            
                            if redundantPriority is None or priority >= redundantPriority:
                                ...
                                ( (cloneSourceOrb, cloneTargetOrb), copyMol) = Orbital.cloneOrbitalsWithCommonMol( [sourceOrb, targetOrb] );
...
class PericyclicBimolecularProposer(BaseReactionProposer):
    ...
    def addPatternListPair(self, sourceTuples, targetTuples, sourceKeyMapIndexes, targetKeyMapIndexes ):
        ...
        for (priority, smartsPattern, orbInfoStr) in sourceTuples:
            matcher = OESubSearch();
            matcher.Init(smartsPattern);
            sourcePatternMatcherList.append( (priority, smartsPattern, matcher, orbInfoStr) );

        targetPatternMatcherList = list();
        for (priority, smartsPattern, orbInfoStr) in targetTuples:
            matcher = OESubSearch();
            matcher.Init(smartsPattern);
            targetPatternMatcherList.append( (priority, smartsPattern, matcher, orbInfoStr) );
        ...
        self.patternMatcherListPairs.append( (sourcePatternMatcherList, targetPatternMatcherList, sourceKeyMapIndexes, targetKeyMapIndexes) );
    ...
    def setupDefaultPatternMatchers(self):
        ...
        (   [   ( 100, "[*:1]([*:10])(/[*:11])=[*:2]/[*:3]=[*:4]([*:12])(/[*:13])", "1 pi 2 2 2,10,11 None; 3 pi 4 2 None 3,12,13"), 
            ...
            [   ( 100, "[*:5](/[*:20])([*:21])=[*:6](\[*:22])([*:23])", "6 pi 5 2 5,22,23 6,20,21; 4 p None 0"),
    ...
    def proposeOrbitalPairs(self, reactionMixture, analyzedMoleculeSmiSet=None ):
        ...
        for (sourcePatternMatcherList, targetPatternMatcherList, sourceKeyMapIndexes, targetKeyMapIndexes) in self.patternMatcherListPairs:

            sourceMatchesIter = ReactionMixturePatternMatchFactory(reactionMixture, sourcePatternMatcherList, sourceKeyMapIndexes);
            for (sourceMatch, sourceOrbInfoStr) in sourceMatchesIter:
                ...
                targetMatchesIter = ReactionMixturePatternMatchFactory(reactionMixture, targetPatternMatcherList, targetKeyMapIndexes);
                for (targetMatch, targetOrbInfoStr) in targetMatchesIter:
                    ...
                    if sourceMol == targetMol:
                        ...
                        if self.compatiblePatternMatches( sourceMatch, targetMatch, sourceKeyMapIndexes, targetKeyMapIndexes ):
                            # Beware, if on the same molecule, these might be based on overlapping
                            #   atoms, which would not make sense to "react" them together.
                            ...
                            sourceOrb = Orbital.fromLabeledMolAndInfoStr(sourceMol, sourceOrbInfoStr);
                            targetOrb = Orbital.fromLabeledMolAndInfoStr(targetMol, targetOrbInfoStr);
                    ...
                    # Generate the orbital objects based on the labeled molecule and info strings.
                    sourceOrb = Orbital.fromLabeledMolAndInfoStr(compositeMol, sourceOrbInfoStr);
                    targetOrb = Orbital.fromLabeledMolAndInfoStr(compositeMol, targetOrbInfoStr);
    ...
    def compatiblePatternMatches(self, sourceMatch, targetMatch, sourceKeyMapIndexes, targetKeyMapIndexes):
    ...
    def __iter__(self):
        for smiles, mixtureComponent in self.reactionMixture.iteritems():
            ...
            if mixtureComponent.currentQuantity() > 0:
                ...
                for (priority, smartsPattern, matcher, orbInfoStr) in self.patternMatcherList:
                    # Every match for a given mixture component
                    for match in matcher.Match(mol,False):
                        ...
                        if redundancyKey not in maxPriorityByRedundancyKey or priority >= maxPriorityByRedundancyKey[redundancyKey]:
...
class ReactionMixtureOrbitalFactory:
    ...
    def __init__(self, reactionMixture, orbitalReactivityCalc, restrictSymmetry=True):
        self.reactionMixture = reactionMixture;
        self.reactiveOrbitalFactory = ReactiveOrbitalFactory(orbitalReactivityCalc, restrictSymmetry);
