...
from chemutils.Common.Util import createAtomMapSmiString
from chemutils.Common.OrbitalModel import orbitalInfo
from chemutils.Common.MolExt import clearAtomMaps, clearMolStereo
...
from chemutils.CombiCDB.Const import ERROR, DISFAVORED
...
class ElectronArrow:
    ...
    def isBondDissociationArrow(self):
        ...
        return (
            len(self.sourceIndexes) == 2
            and len(self.targetIndexes) == 1
            and self.targetIndexes[0] in self.sourceIndexes
        )
    ...
    def isBondPivotArrow(self):
        ...
        return (
            len(self.sourceIndexes) == 2
            and len(self.targetIndexes) == 2
            and len(set(self.sourceIndexes).intersection(set(self.targetIndexes))) == 1
        )
    ...
    def apply(self, mol, halfBondIndexes=None):
        ...
        """
        log.debug(
            "%s %s %s" % (createStandardSmiString(mol), str(self), halfBondIndexes)
        )
    ...
    def parseArrowCode(arrowCode):
        ...
        http://baldigwiki.ics.uci.edu/index.php/Web_Development#Details_on_.27arrow_description.27_syntax
        ...
        """
        nElectrons = 0
        # Number of electrons moving according to the arrow
    ...
    def makeCanonicalArrow(self):
        ...
        if len(self.sourceIndexes) == 2:
            commonElem = list(
                set(self.sourceIndexes).intersection(set(self.targetIndexes))
            )
            if len(commonElem) == 1:
                commonElem = commonElem[0]
                if commonElem in self.sourceIndexes:
                    self.sourceIndexes = tuple(
                        [
                            self.sourceIndexes[
                                1 - list(self.sourceIndexes).index(commonElem)
                            ],
                            commonElem,
                        ]
                    )
                if len(self.targetIndexes) == 2 and commonElem in self.targetIndexes:
                    self.targetIndexes = tuple(
                        [
                            commonElem,
                            self.targetIndexes[
                                1 - list(self.targetIndexes).index(commonElem)
                            ],
                        ]
                    )
    ...
    def parseArrowCodes(mol, arrowCodes):
        ...
        for arrowCode in arrowCodeList:
            ...
            arrowObjList.append(
                ElectronArrow(mol, sourceIndexes, nElectrons, targetIndexes)
            )
        return arrowObjList
    ...
    def prepareArrowCodes(arrowList):
        ...
        for arrowCode, arrow in arrowsByCode.items():
            comboArrow = None
            if (
                len(arrow.sourceIndexes) == 2
                and len(arrow.targetIndexes) == 1
                and arrow.targetIndexes[0] in arrow.sourceIndexes
            ):
        ...
        arrowCodeList = sorted(arrowCodeList)
...
def createNonIsoSmi(mol):
    ...
    """
    mol = OEGraphMol(mol)
    # Create a copy we're free to modify
    ...
    for atom in mol.GetAtoms():
        if (
            atom.GetAtomicNum() == 1
            and atom.GetMapIdx() > 0
            and atom.GetFormalCharge() == 0
        ):
            atom.SetAtomicNum(0)
    smi = OECreateCanSmiString(mol)
    # Stereochemistry not yet supported for arrow diagrams
    smi = standardizeSmiles(smi)
    # Strange step.  Some cases with labeled hydrogens and "fake" stereocenters, canonical smi algorithm messes up
    smi = smi.replace("[R", "[H:")
    return smi
...
def prepLabeledSmi(labeledSmi):
    ...
    """
    return labeledSmi.replace("[1H", "[H")
    # Get rid of the artificial 1H isotopes at this point


def reactionProfileSupportsMechanism(reactionProfile):
    return (
        reactionProfile["mechanism_arrow_codes"] is not None
        and reactionProfile["mechanism_arrow_codes"] != ""
        and reactionProfile["warning_level_id"] not in (ERROR, DISFAVORED)
    )


def generateMechanismPairs(
    reactants, reagent, reactionProfile, reactionProcessor=None, clearLabels=False
):
    ...
    labeledReactantList = reactionProcessor.applyReactionBySmirks(
        label_smirks, extReactants
    )
    for labeledReactant in labeledReactantList:
        ...
        labeledProduct = labeledReactant
        # OEGraphMol(labeledReactant);
...
def moveAtomElectrons(
    sourceAtom, targetAtom, electronCount=2, arrowObjList=None, halfBondIndexes=None
):
    ...
    if bond is not None:
        ...
        if sourceAtom.GetParent() != targetAtom.GetParent():
            ...
            raise Exception(
                "Attempting to move electrons between atoms from separate molecule graph objects"
            )
    ...
    if arrowObjList is not None:
        ...
        if electronCount == 1:
            ...
            for arrowObj in arrowObjList:
                if (
                    len(arrowObj.sourceIndexes) == 2
                    and matchSource in arrowObj.sourceIndexes
                    and matchTarget == arrowObj.targetIndexes
                ):
                    haveFoundMatch = True
                    break
            ...
            if haveFoundMatch:
                arrow = ElectronArrow(
                    ...
                    sourceIndexes,
                    electronCount,
                    [sourceIndexes[0], targetIndexes[0]],
                )
            else:
                # CASE 2:
                ...
                for i, arrowObj in enumerate(arrowObjList):
                    if (
                        len(arrowObj.sourceIndexes) == 1
                        and len(arrowObj.targetIndexes) == 1
                        and arrowObj.sourceIndexes[0] == targetIndexes[0]
                        and arrowObj.targetIndexes[0] == sourceIndexes[0]
                    ):
                        idxToFix = i
                        break
                ...
                if idxToFix != -1:
                    ...
                    arrow = ElectronArrow(
                        mol, sourceIndexes, electronCount, targetIndexes
                    )
                    newOldArr = ElectronArrow(
                        mol, oldArrow.sourceIndexes, electronCount, targetIndexes
                    )
...
def moveBondElectrons(
    bond, atom, electronCount=2, pivotAtom=None, arrowObjList=None, halfBondIndexes=None
):
    ...
    if neighborAtom is not None:
        ...
        moveBondElectrons(
            bond, pivotAtom, electronCount, halfBondIndexes=halfBondIndexes
        )
        moveAtomElectrons(
            pivotAtom, atom, electronCount, halfBondIndexes=halfBondIndexes
        )
    else:
        raise Exception(
            "Ambiguous scenario, don't know which source atom of the bond to leave the new bond attached to.  Call moveBondElectrons to one of the attached atoms first.\n%s %s,%s %s"
            % (
        ...
        if electronCount == 1:
            ...
            for i, arrowObj in enumerate(arrowObjList):
                if (
                    len(arrowObj.sourceIndexes) == 1
                    and len(arrowObj.targetIndexes) == 1
                    and arrowObj.sourceIndexes[0] == atom.GetMapIdx()
                    and arrowObj.targetIndexes[0] == pivotAtom.GetMapIdx()
                ):
                    idxOfFix = i
                    break
            if idxOfFix != -1:
                wrongArrow = arrowObjList.pop(idxOfFix)
                fixedArrow = ElectronArrow(
                    mol, wrongArrow.sourceIndexes, electronCount, targetIndexes
                )
...
class BondDoesNotExistException(Exception):
    ...
    def __str__(self, atom=None, pivotAtom=None, electronCount=None):
        ...
        return """Attempted to move electrons from a bond that no longer exists.  
...
def determineTransitionArrows(
    sourceMol,
    targetMol,
    arrowObjList=None,
    sourceAtomByMapIdx=None,
    targetAtomByMapIdx=None,
    sourceIndexSetsUsed=None,
):
        ...
        if diffCharge == +1:
            ...
            for neighborMapIdx, sourceBond in sourceBondByNeighborMapIdx.items():
                ...
                if targetBond is None or targetBond.GetOrder() < sourceBond.GetOrder():
                    ...
                    arrowObj = ElectronArrow(
                        sourceMol, [mapIdx, neighborMapIdx], 2, [neighborMapIdx]
                    )
                    isNewSource = applyAddArrow(
                        targetMol, arrowObj, arrowObjList, sourceIndexSetsUsed
                    )
                    if isNewSource:
                        # Found a valid new arrow to apply.  target structure has changed, need to reevaluate from the top.  Can stop the current loop
                        arrowObjList.extend(
                            determineTransitionArrows(
                                sourceMol,
                                targetMol,
                                arrowObjList,
                                sourceAtomByMapIdx,
            ...
            for neighborMapIdx, targetBond in targetBondByNeighborMapIdx.items():
                ...
                if sourceBond is None or sourceBond.GetOrder() < targetBond.GetOrder():
                    ...
                    isNewSource = applyAddArrow(
                        sourceMol, arrowObj, arrowObjList, sourceIndexSetsUsed
                    )
                    if isNewSource:
            ...
            for neighborMapIdx, sourceBond in sourceBondByNeighborMapIdx.items():
                ...
                if targetBond is None or targetBond.GetOrder() < sourceBond.GetOrder():
                    ...
                    if sourceAtom.GetFormalCharge() > sourceNeighbor.GetFormalCharge():
                        ...
                        sourceNeighbor.GetFormalCharge() == sourceAtom.GetFormalCharge()
                    ):
                        # Same formal charge, so move in direction of general electronegativity
                        if atomElectronegativity(
                            sourceAtom.GetAtomicNum()
                        ) > atomElectronegativity(sourceNeighbor.GetAtomicNum()):
                            sinkMapIdx = mapIdx

                    arrowObj = ElectronArrow(
                        sourceMol, [mapIdx, neighborMapIdx], 2, [sinkMapIdx]
                    )
                    isNewSource = applyAddArrow(
                        targetMol, arrowObj, arrowObjList, sourceIndexSetsUsed
                    )
    ...
    if sourceSmi != targetSmi:
        raise Exception(
            "Unable to find arrow to convert source into target structure: %s vs. %s"
            % (sourceSmi, targetSmi)
        )
    ...
    if origSourceMol is not None:
        ...
        arrowCodes = ElectronArrow.prepareArrowCodes(arrowObjList)
        # Added benefit of merging extra arrows that pass through pivot atoms
