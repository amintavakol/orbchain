...
class ReagentManager:
    ...
    def __init__(self):
        ...
        self.objectCache = dict();  # Replace with external one as appropriate
    ...
    def loadDBInstances( self, reagentSearchModel=None, conn=None ):
        ...
        if not extConn:
            if self.connFactory is not None:
                ...
                conn = DBUtil.connection(); # Default connection source

        if reagentSearchModel is None:
            reagentSearchModel = ReagentSearchModel();  # Just generate a new one with default options
        ...
        try:
            ...
            if len(reagentModels) > 0 and not reagentSearchModel.shallowReagents:
                ...
                reactionTable = DBUtil.execute( query, includeColumnNames=True, conn=conn );
                ...
                for reactionModel in reactionModels:
                    if SMIRKSReagent.smirksCodec.isEncrypted(reactionModel["smirks"]):
                        reactionModel["smirks"] = SMIRKSReagent.smirksCodec.decrypt(reactionModel["smirks"]);
                    if SMIRKSReagent.smirksCodec.isEncrypted(reactionModel["mechanism_label_smirks"]):
                        reactionModel["mechanism_label_smirks"] = SMIRKSReagent.smirksCodec.decrypt(reactionModel["mechanism_label_smirks"]);
                ...
                if len(inheritedReagentTable) > 0:
                    ...
                    inheritedReagents = self.loadDBInstances( inheritedReagentSearchModel, conn );
                    inheritedReagentsById = modelDictFromList(inheritedReagents, "reagent_id");
                    ...
                    for row in inheritedReagentTable:
                        ...
                        for reactionProfile in inherited_reagent.reactionProfileList:
                            copyReactionProfile = dict(reactionProfile);    # Create a copy we can modify
                            copyReactionProfile["reagent_id"] = primary_reagent_id; # Alter the ownership of the copy from the inherited reagent to the primary (inheriting) reagent
                            ...
                            inheritedReactionKey = SMIRKSReagent.reactionModelCompositeKey(copyReactionProfile);
                ...
                for reactionModel in reactionModels:
                    ...
                    if reactionModel["description"] is None: reactionModel["description"] = "";
                    if reactionModel["mechanism_label_smirks"] is None: reactionModel["mechanism_label_smirks"] = "";
                    if reactionModel["mechanism_arrow_codes"] is None: reactionModel["mechanism_arrow_codes"] = "";
                ...
                if "smirks_list" in reagentModel:
                    for key in reagentModel.iterkeys():
                        if key.endswith(LIST_SUFFIX):
                            reagentModel[key] = str.join(REACTION_PROFILE_DELIM, reagentModel[key]);
    ...
    def retrieveRetroReagentsById( self, neededRetroReagentIds, conn=None):
        ...
        retroReagentsById = dict();  # Container to track loaded object
        ...
        newRetroReagentIds = loadObjectsFromQueuesById(retroReagentsById, 
                                    retroReagentQueuesById, neededRetroReagentIds);
        
        log.debug("Loaded %d Retro Reagents From cache" % (len(neededRetroReagentIds) - len(newRetroReagentIds)))
    ...
    def retrieveShallowReagentsByReactionCategoryIds( self, reactionCategoryIDs ):
        ...
        if "reagentIdsByReactionCategoryId" not in self.objectCache:
            ...
            for row in reagentData:
                ...
                if reactionCategoryID not in reagentIdsByReactionCategoryId:
                    reagentIdsByReactionCategoryId[reactionCategoryID] = set();    # Initialize empty set for this category
        ...
        if reactionCategoryIDs is not None:
            for reactionCategoryID in reactionCategoryIDs:
                reactionCategoryID = int(reactionCategoryID);   # Ensure proper type for dictionary lookup
                if reactionCategoryID in reagentIdsByReactionCategoryId:
                    expectedReagentIds.update( reagentIdsByReactionCategoryId[reactionCategoryID] );
        else:
    ...
    def retrieveShallowReagents(self):
        ...
        if "shallowReagents" not in self.objectCache:
            ...
            reagentQuery.shallowReagents = True;    # Just for display and selection purposes here
    ...
    def applyReagent( self, reactantList, reagent ):
        ...
        key = "%s>%s>" % (str.join(MOL_LIST_DELIM, molToSmiList(reactantList)), reagent["reagent_id"]);
        ...
        if self.resultCache is not None and key in self.resultCache:
            ...
            if self.resultCache is not None:
                self.resultCache[key] = result; # Store in result cache if available for future use
            return result;
