...
from openeye.oechem import OELibraryGen, OESubSearch;
...
from chemutils.Common.Util import virtual_oemolistream, virtual_oemolostream, molBySmiles;
from chemutils.Common.Util import createStandardSmiString, standardizeSmiles;
from chemutils.Common.Util import splitCompositeMol, splitCompositeMolToSmilesList, splitCompositeSmilesToList, molToSmiList, atomCount;
from chemutils.Common.Util import Codec;
from chemutils.Common.Model import RowItemModel, modelListFromTable, modelDictFromList, SQLQuery;
from chemutils.Common import DBUtil;
from chemutils.Common.MolExt import enumerateRacemicMixture, enumerateRacemicMixtureList, createStandardMol, isCarbonRadical;
from ReactionProcessor import ReactionProcessor;
from Util import log;
from chemutils.CombiCDB.Const import REACTION_DELIM, MOL_LIST_DELIM, REACTION_PROFILE_DELIM, VALUE_DELIM;
from chemutils.CombiCDB.Const import SENTINEL_CHARGE, SENTINEL_OPTIONAL_STEP_CHARGE, NORMAL_CHARGE_THRESHOLD, SENTINEL_REJECT_CHARGE, SENTINEL_REJECT_IMMEDIATE_CHARGE;
...
from chemutils.CombiCDB.Const import OK, CAUTION, WARNING, ERROR, DISFAVORED, RETRO_ONLY;
...
class BaseReagent(RowItemModel):
    ...
    def loadTextProperties(self):
        ...
        if self["inherent_reactant_smiles"] is not None:
            self.inherentReactants = [];
            smilesList = splitCompositeSmilesToList(self["inherent_reactant_smiles"], retainCounterIons=False);
        ...
        if self["inherent_product_smiles"] is not None:
            ...
            smilesList = splitCompositeSmilesToList(self["inherent_product_smiles"], retainCounterIons=False);
            for smiles in smilesList:
                if len(smiles.strip()) > 0:
    ...
    def __call__(self, reactants, supplementalData=None, enumerateRacemicMixtures=True ):
        ...
        for reactant in reactants:
            reactantSmiList.extend( splitCompositeMolToSmilesList(reactant, retainCounterIons=False) );
        ...
        if enumerateRacemicMixtures:
            ...
            for stereoSpecifiedReactants in enumerateRacemicMixtureList(reactants):
                ...
                products.extend( self(stereoSpecifiedReactants, supplementalData, enumerateRacemicMixtures=False) );
        else:
            # Standard generation run
            products = self.generateProducts( reactants, supplementalData );

            if self.isRetro() and (self.enforceRetroReactions or supplementalData is not None):
                ...
                for iProduct in xrange(len(products)-1, -1, -1):
                    ...
                    forwardReactantCompositeSmi = createStandardSmiString(forwardReactantComposite);

                    # Reapply these after dumping inherent reactants, and separating out composites
                    forwardReactantSmiList = splitCompositeMolToSmilesList(forwardReactantComposite, retainCounterIons=False);
                    forwardReactantList = [];
                    for forwardReactantSmi in forwardReactantSmiList:
                        if forwardReactantSmi not in inherentReactantSmiSet:
                            ...
                            log.debug("Forward Reactant: "+ createStandardSmiString(forwardReactant) );

                    # Generate the products.  
                    forwardProducts = self.originalReagent(forwardReactantList);
                    supplementalData.retroValidationProductsByPrecursorSmi[forwardReactantCompositeSmi] = forwardProducts; # Link a copy accessible to the caller as a member variable

                    # Our original (retro) reactants had better be amongst these
                    retroReactantsFound = False;
                    for forwardProduct in forwardProducts:
                        forwardProductSmiList = splitCompositeMolToSmilesList(forwardProduct, retainCounterIons=False);
                    ...
                    supplementalData.retroReactantsReproduced = supplementalData.retroReactantsReproduced or retroReactantsFound;
    ...
    def postProcessing(self, products, reactantSmiSet, supplementalData):
        ...
        productSmiSet = set();  # Check for redundancies
        extraProducts = []; # Any extra products generated, perhaps by chirality enumeration
        
        for iProduct in xrange(len(products)-1,-1,-1):    # Reverse iteration in case need to delete any items
            ...
            if productSmi in productSmiSet:
                products.pop(iProduct); # Already in product set, remove redundant copy
                continue;
            else:
    ...
    def postProcess(self, product, extraProducts=None, reactantSmiSet=None, allowIntermediate=None):
        ...
        changesMade = changesMade1 or changesMade2; # Must separate OR logic, otherwise may short-circuit logic and skip the method call
        ...
        componentProductSmiList = splitCompositeMolToSmilesList(product, retainCounterIons=True);
        ...
        for iComponent in xrange(len(componentProductSmiList)-1,-1,-1):
            ...
            componentCharge = OENetCharge(mol);
            if not rejectComponent and not allowIntermediate and componentCharge * netCharge > 0:
        ...
        if componentsRemoved:
            if len(componentProductSmiList) < 1:
                ...
                newProduct = molBySmiles(str.join(SMILES_MOL_DELIM, componentProductSmiList));
                ...
                for iCopy, copyProduct in enumerate(enumerateRacemicMixture(newProduct)):
...
class SMIRKSReagent(BaseReagent):
    ...
    def reactionModelCompositeKey( reactionModel ):
        ...
        """
        return (reactionModel['reagent_id'], reactionModel['reaction_profile_id'], reactionModel['priority']);
    ...
    def loadTextProperties(self):
        ...
        if self["reaction_profile_id_list"] is not None:
            ...
            for reaction_profile_idStr in self["reaction_profile_id_list"].split(REACTION_PROFILE_DELIM):
                reaction_profile_id = None;
                try:
                    reaction_profile_id = int(reaction_profile_idStr)
                except:
                    reaction_profile_id = 0;   # Default value if none specified
        ...
        if self["reagent_reaction_profile_id_list"] is not None:
            ...
            for reagent_reaction_profile_idStr in self["reagent_reaction_profile_id_list"].split(REACTION_PROFILE_DELIM):
                reagent_reaction_profile_id = None;
                try:
                    reagent_reaction_profile_id = int(reagent_reaction_profile_idStr)
                except:
                    reagent_reaction_profile_id = 0;   # Default value if none specified
        ...
        if self["priority_list"] is not None:
            ...
            for priorityStr in self["priority_list"].split(REACTION_PROFILE_DELIM):
                ...
                try:
                    ...
                    priority = 0;   # Default value if none specified
        ...
        if self["warning_level_id_list"] is not None:
            warningLevelList = [];
            for warningLevelStr in self["warning_level_id_list"].split(REACTION_PROFILE_DELIM):
                warning_level_id = None;
                try:
                    warning_level_id = int(warningLevelStr)
                except:
                    warning_level_id = 0;   # Default value if none specified
                warningLevelList.append( warning_level_id );

        warningMessageList = None;
        if self["warning_message_list"] is not None:
            warningMessageList = [];
            for warning_message in self["warning_message_list"].split(REACTION_PROFILE_DELIM):
        ...
        if self["reaction_category_ids_list"] is not None:
            ...
            for reactionCategoryIDsStr in self["reaction_category_ids_list"].split(REACTION_PROFILE_DELIM):
        ...
        if self["pre_status_threshold_list"] is not None:
            ...
            for preStatusThresholdStr in self["pre_status_threshold_list"].split(REACTION_PROFILE_DELIM):
                pre_status_threshold = None;
                try:
                    pre_status_threshold = int(preStatusThresholdStr)
                except:
                    pre_status_threshold = 0;   # Default value if none specified
        ...
        if self["pre_status_minimum_list"] is not None:
            ...
            for preStatusMinimumStr in self["pre_status_minimum_list"].split(REACTION_PROFILE_DELIM):
                pre_status_minimum = None;
                try:
                    pre_status_minimum = int(preStatusMinimumStr)
                except:
                    pre_status_minimum = 0;   # Default value if none specified
        ...
        if self["post_status_list"] is not None:
            ...
            for postStatusStr in self["post_status_list"].split(REACTION_PROFILE_DELIM):
                ...
                try:
                    ...
                    post_status = 0;   # Default value if none specified
        ...
        if self["intended_usage_list"] is not None:
            intendedUsageList = [];
            for intendedUsageStr in self["intended_usage_list"].split(REACTION_PROFILE_DELIM):
        ...
        if self["mechanism_label_smirks_list"] is not None and len(self["mechanism_label_smirks_list"].strip()) > 0:
            mechanism_label_smirksList = self["mechanism_label_smirks_list"].split(REACTION_PROFILE_DELIM);

        mechanism_arrow_codesList = None;
        if self["mechanism_arrow_codes_list"] is not None and len(self["mechanism_arrow_codes_list"].strip()) > 0:
            mechanism_arrow_codesList = self["mechanism_arrow_codes_list"].split(REACTION_PROFILE_DELIM);
        ...
        if self["smirks_list"] is not None:
            ...
            for i, smirks in enumerate(smirksList):
                ...
                reactionProfile["reagent_reaction_profile_id"] = reagent_reaction_profile_id;
                ...
                self.intendedUsageSpecified = self.intendedUsageSpecified or intended_usage;
    ...
    def generateProducts(self, reactants, supplementalData=None):
        ...
        finalProductList = self.__recursiveGeneration( reactants, trackingData=trackingData, supplementalData=supplementalData );
    ...
    def retro(self):
        ...
        reaction profiles.
        """
        retroProps = dict(self);    # Copy over initialization properties
        ...
        for reactionProfile in self.reactionProfileList:
            if reactionProfile["retro_ready"]:
                ...
                retroPreStatusMinimumList.append(str(0));   # Necessary to allow reaction chain to initiate, when assume start with status = 0
                retroPostStatusList.append(str((preStatusThreshold+preStatusMinimum)/2));
            
        retroProps["smirks_list"] = str.join(REACTION_PROFILE_DELIM, retroSmirksList );
        retroProps["description_list"] = str.join(REACTION_PROFILE_DELIM, retroDescriptionList );
        retroProps["priority_list"] = str.join(REACTION_PROFILE_DELIM, retroPriorityList );
        retroProps["pre_status_threshold_list"] = str.join(REACTION_PROFILE_DELIM, retroPreStatusThresholdList );
        retroProps["pre_status_minimum_list"] = str.join(REACTION_PROFILE_DELIM, retroPreStatusMinimumList );
        retroProps["post_status_list"] = str.join(REACTION_PROFILE_DELIM, retroPostStatusList );
    ...
    def __recursiveGeneration(self, reactants, depth=0, requiredReactant=None, trackingData=None, supplementalData=None ):
        ...
        if depth >= self["recursions"]:  return [];
        ...
        if smiListStr not in trackingData.recursiveReactantSmiDict:
            ...
            for iReactant, reactant in enumerate(reactants):
                log.debug("Reactant %d: %s" % (iReactant, createStandardSmiString(reactant) ) );

            # Main action
            (alreadyGenerated, productList) = self.__singleStepGeneration( reactants, requiredReactant, trackingData=trackingData, supplementalData=supplementalData );

            # See if we should do any more recursions, and filter product list to acceptable ones
            acceptedProductList = [];   # Weird, should be able to use original productList and pop unwanted items, but bug seems to result in empty lists unexpectedly
            for product in productList:
                productSmiles = createStandardSmiString(product);

                # Resolve into component products if mixture
                productSmilesList = splitCompositeSmilesToList(productSmiles, retainCounterIons=False);
                ...
                reactants = []; # Next recursion reactant container
                ...
                for componentProductSmiles in productSmilesList:
                    ...
                    componentProduct.SetIntData("warning_level_id", product.GetIntData("warning_level_id") );   # Propagate warning data
                    componentProduct.SetStringData("warning_message", product.GetStringData("warning_message") );   # Propagate warning data
                    componentProduct.SetIntData("post_status", product.GetIntData("post_status") );
                    componentProduct.SetIntData("priority", product.GetIntData("priority") );
                    reactants.append( componentProduct );    # Use each of last recursion's products as next reactant

                    componentSubProducts = [];

                    if self["possible_reactants"] < 2:
                        componentSubProducts.extend( self.__recursiveGeneration( reactants, depth+1, trackingData=trackingData, supplementalData=supplementalData ) );
                        ...
                        preReactionOptionalCharges = countAtomCharges(reactants, SENTINEL_OPTIONAL_STEP_CHARGE);
                        unimolecularProducts = self.__recursiveGeneration( reactants, depth+1, trackingData=trackingData, supplementalData=supplementalData );
                        postReactionOptionalCharges = countAtomCharges(unimolecularProducts, SENTINEL_OPTIONAL_STEP_CHARGE);
                        componentSubProducts.extend( unimolecularProducts );

                        if len(unimolecularProducts) < 1 or postReactionOptionalCharges - preReactionOptionalCharges > 0:
                            ...
                            for originalReactant in trackingData.originalReactantMolList:
                                ...
                                subPriority = -1;
                                (alreadyGenerated, subProductList) = self.__singleStepGeneration( reactants, componentProduct, trackingData=trackingData, supplementalData=supplementalData );
                                for subProduct in subProductList:
                                    subPriority = max(subPriority, subProduct.GetIntData("priority") );
                                if subPriority not in originalReactantByPriority:
                                    originalReactantByPriority[subPriority] = list();
                                originalReactantByPriority[subPriority].append( originalReactant );
                                topSubPriority = max(topSubPriority, subPriority);
                                
                                reactants.pop();    # Revert state
                            ...
                            if topSubPriority in originalReactantByPriority:
                                for originalReactant in originalReactantByPriority[topSubPriority]:
                                    reactants.append( originalReactant );
                                    componentSubProducts.extend( self.__recursiveGeneration( reactants, depth+1, componentProduct, trackingData=trackingData, supplementalData=supplementalData ) );
                                    reactants.pop();    # Revert state
                    reactants.pop();    # Restore state                

                    componentSubProductsByComponentSmi[componentProductSmiles] = componentSubProducts;
                ...
                if self.countIntermediates:
                    ...
                    self.__addToFinalProductList( product, productSmilesList, acceptedProductList, depth, trackingData=trackingData, supplementalData=supplementalData )
                ...
                if len(allSubProducts) > 0:
                    ...
                    for componentProductSmiles in productSmilesList:
                        ...
                        if componentProductSmiles in componentSubProductsByComponentSmi:
                            lookupProducts = componentSubProductsByComponentSmi[componentProductSmiles];
                            if len(lookupProducts) > 0:
                                componentSubProducts = lookupProducts;
                        componentSubProductsList.append( componentSubProducts );

                    for compositeSubProduct in self.__allComponentCombinations( componentSubProductsList ):
                        subProductSmiList = splitCompositeMolToSmilesList(compositeSubProduct, retainCounterIons=False);
                        self.__addToFinalProductList( compositeSubProduct, subProductSmiList, acceptedProductList, depth, trackingData=trackingData, supplementalData=supplementalData );
    ...
    def __singleStepGeneration(self, reactants, requiredReactant=None, trackingData=None, supplementalData=None):
        ...
        for reactant in reactants:
            (reactant_warning_level_id, reactant_warning_message) = \
                self.__worstWarningLevel \
                (   reactant_warning_level_id, reactant_warning_message, 
                    reactant.GetIntData("warning_level_id"), reactant.GetStringData("warning_message")
                );
        ...
        reactantSmiList = molToSmiList( reactants );
        smiListStr = str.join(MOL_LIST_DELIM, reactantSmiList);
        smiListStr += "%s%s%s%s" % (MOL_LIST_DELIM, extremeReactantPostStatus, MOL_LIST_DELIM, requiredReactantSmi);
        ...
        if not alreadyGenerated:
            ...
            disfavoredProductsPriority = -1;    # Priority of the last reaction profile found which yielded "disfavored" products
            for reactionIndex, reactionProfile in enumerate(self.reactionProfileList):
                ...
                acceptableReactionProfile = topPriorityWithProducts is None or priority >= topPriorityWithProducts
                acceptableReactionProfile = acceptableReactionProfile and extremeReactantPostStatus <= reactionProfile["pre_status_threshold"];
                acceptableReactionProfile = acceptableReactionProfile and extremeReactantPostStatus >= reactionProfile["pre_status_minimum"];
                acceptableReactionProfile = acceptableReactionProfile and not (reactionProfile["warning_level_id"] == DISFAVORED and reactionProfile['priority'] < disfavoredProductsPriority );  # If already found some disfavored products, don't keep looking for more down the priority list
                acceptableReactionProfile = acceptableReactionProfile and (reactionProfile["warning_level_id"] != RETRO_ONLY or self.isRetro());

                if acceptableReactionProfile:
                    productString = None;
                    productCount = None;
                    
                    lockAcquired = threadLock.acquire(False);   # Verify the libgen is available for use and not locked by another thread
                    try:
                        if not lockAcquired:
                        ...
                        productCount = self.reactionProcessor.applyReaction( libgen, extendedReactants, productOEOS, reactionIndex, rejectProductSmiSet=rejectProductSmiSet );
                    ...
                    if productCount > 0:
                        (worst_warning_level_id, worst_warning_message) = \
                            self.__worstWarningLevel \
                            (   reactant_warning_level_id, reactant_warning_message, 
                                reactionProfile["warning_level_id"], reactionProfile["warning_message"]
                            );
                        ...
                        for product in productOEIS.GetOEGraphMols():
                            ...
                            componentProductSmiList = splitCompositeMolToSmilesList(product, retainCounterIons=False);
                            ...
                            for iComponent in xrange(len(componentProductSmiList)-1, -1, -1):   # Reverse iteration for safety since may be deleting items
                                componentProductSmi = componentProductSmiList[iComponent];
                                OEParseSmiles(componentMol, componentProductSmi);
                                rejectComponent = False;

                                if not rejectComponent and self.removeInherentProducts and self.inherentProducts is not None:
                                    # Discard inherent products
                                    if componentProductSmi in self.inherentProductSmiSet:
                                        rejectComponent = True;
                                    else:
                                        # Check if the post-processed version is there
                                        OEParseSmiles(componentCopy, componentProductSmi);
                                        componentProcessed = self.postProcess(componentCopy, allowIntermediate=True);
                                        componentCopySmi = createStandardSmiString(componentProcessed);
                                        if componentCopySmi in self.inherentProductSmiSet:
                                            rejectComponent = True;
                                        componentCopy.Clear();

                                if not rejectComponent and componentProductSmi in rejectProductSmiSet:
                                ...
                                if not rejectComponent:
                                    ...
                                    for atom in componentMol.GetAtoms():
                                        if atom.GetFormalCharge() == SENTINEL_REJECT_IMMEDIATE_CHARGE:
                                            rejectComponent = True;
                            ...
                            if rejectAll:
                                ...
                                productSmi = str.join(SMILES_MOL_DELIM, componentProductSmiList)
                                if productSmi != "":
                                    ...
                                    for atom in product.GetAtoms():
                                        if atom.GetAtomicNum() in self.requiredAtomicNums:
                                            requiredAtomFound = True;
                                            break;
                                    if not requiredAtomFound:
                                        product = None; # No useful components left, reject product
                                else:
                                    product = None; # All components rejected, no product left
                            ...
                            if product is not None:
                                ...
                                for productCopy in enumerateRacemicMixture(product):
                                    productCopySmi = createStandardSmiString(productCopy);
                                    if productCopySmi not in currentProductSmiSet:  # Don't bother adding redundant products
                                        productCopy.SetIntData("warning_level_id", worst_warning_level_id);
                                        productCopy.SetStringData("warning_message", worst_warning_message);
                                        if reactionProfile["post_status"] >= 0:
                                            productCopy.SetIntData("post_status", reactionProfile["post_status"] );
                                        else:
                                            # Negative post_status doesn't make sense.  Sentinel value to just propagate last status
                                            productCopy.SetIntData("post_status", extremeReactantPostStatus );
                                        productCopy.SetIntData("priority", reactionProfile["priority"] );
                                        ...
                                        if worst_warning_level_id == DISFAVORED:
                                            ...
                                            productCopy = self.postProcess(productCopy, allowIntermediate=True);
                                            productCopySmi = createStandardSmiString(productCopy);
                                            if productCopy is not None and productCopySmi not in supplementalData.disfavoredProductDict:
                                                supplementalData.disfavoredProductDict[productCopySmi] = productCopy;
                                                elementaryStep = ( reactantsCopy, reactionProfile, [productCopy]);
                                                supplementalData.elementarySteps.append( elementaryStep );

                                                # Second copy which does not allow intermediate (i.e., net charged or radical structures)
                                                nonIntermedCopy = OEGraphMol(productCopy);
                                                nonIntermedCopy = self.postProcess(nonIntermedCopy, allowIntermediate=False);
                                                nonIntermedCopySmi = createStandardSmiString(nonIntermedCopy);
                                                if nonIntermedCopy is not None and nonIntermedCopySmi not in supplementalData.disfavoredProductDict:
                                                    supplementalData.disfavoredProductDict[nonIntermedCopySmi] = nonIntermedCopy;
                                                    elementaryStep = ( reactantsCopy, reactionProfile, [nonIntermedCopy]);
                                                    supplementalData.elementarySteps.append( elementaryStep );
                                        ...
                                        if productCopy is not None:
                                            log.debug( "Product Generated: %s, by SMIRKS %s %s %s" % (createStandardSmiString(productCopy), reactionProfile["description"], reactionProfile["priority"], productCopy.GetIntData("post_status")) )
                                ...
                                if len(reactionProfileProducts) > 0:
                                    elementaryStep = ( reactantsCopy, reactionProfile, reactionProfileProducts);
                                    supplementalData.elementarySteps.append( elementaryStep );
            ...
            log.debug("Products already generated for reactants: %s >> %s" % (smiListStr, productSmiListStr) );

        return ( alreadyGenerated, trackingData.reactantSmilesDict[smiListStr] );

    def __addToFinalProductList( self, product, productSmilesList, acceptedProductList, depth, trackingData=None, supplementalData=None ):
        productSmi = createStandardSmiString(product);
        if productSmi not in trackingData.compositeProductSmilesSet:
            log.debug("Adding to accepted product list at depth %d: %s %d" % (depth, str(productSmilesList), product.GetIntData("warning_level_id")) );
    ...
    def __allComponentCombinations( self, componentSubProductsList, currentList=None ):
        ...
        if currentPosition >= len(componentSubProductsList):
            ...
            for componentProduct in currentList:
                (warning_level_id, warning_message) = \
                    self.__worstWarningLevel \
                    (   warning_level_id, warning_message, 
                        componentProduct.GetIntData("warning_level_id"), componentProduct.GetStringData("warning_message")
                    );
            ...
            for componentProduct in nextComponentSubProducts:
                ...
                for compositeProduct in self.__allComponentCombinations( componentSubProductsList, currentList ):
                    yield compositeProduct;
                currentList.pop();  # Restore state
    ...
    def __worstWarningLevel( self, level1, message1, level2, message2 ):
        ...
        """
        if message1 is None or message1 == NULL_STRING: message1 = "";
        if message2 is None or message2 == NULL_STRING: message2 = "";
        comboMessage = "";
        if level2 > level1:
            comboMessage = message2;
            if not message1 in message2:
                ...
                if len(comboMessage) > 0 and len(message1) > 0:
                    ...
                    comboMessage += message1;   # One or the other is blank, no point in adding <br> line break
            return (level2, comboMessage);
        else:
            comboMessage = message1;
            if not message2 in message1:
                # If the next message has not already been observed, then append it
                if len(comboMessage) > 0 and len(message2) > 0:
                    comboMessage += "<br>"+ message2;
                else:
                    comboMessage += message2;   # One or the other is blank, no point in adding <br> line break
...
class TrackingDataModel:
    ...
    reactantSmilesDict = None;    # Keyed by reactant list string, value is list of product molecules that came out of it
    recursiveReactantSmiDict = None;  # Same as above but for recursive generation calls, not just single step
    ...
    def __init__(self):
        ...
        """
        self.reactantSmilesDict = dict();    # Keyed by reactant list string, value is list of product molecules that came out of it
        self.recursiveReactantSmiDict = dict();  # Same as above but for recursive generation calls, not just single step
            ...
            for iStep, elementaryStep in enumerate(self.elementarySteps):
                ...
                for detailProduct in detailProducts:
                    ...
                    if detailProductSmi in self.disfavoredProductDict:
                        ...
                        componentSmiSet = set(splitCompositeMolToSmilesList(detailProductCopy));
...
class ReactionStep(list):
    ...
    def __init__(self):
        list.__init__(self);
        self.append(["*"]); # Reactant list with placeholder * molecule
        self.append(None);  # Null reagent object
        self.append(["*"]); # Product list with placeholder * molecule
