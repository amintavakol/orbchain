...
from chemutils.CombiCDB.proposers.AtomBasedReactionProposer import (AtomBasedReactionProposer,
...
class RadicalProposer(AtomBasedReactionProposer):
    ...
    def __init__(self, srcAtomList, sinkAtomList, orbitalPairFilters=defaultFilters, photo=True, checkIntra=True):
        """Basic constructor - call super and set the photo attribute
        """
        self.photo = photo
        super(RadicalProposer, self).__init__(srcAtomList, sinkAtomList, orbitalPairFilters, checkIntra=checkIntra, checkRadical=True)


    def setOrbitalFactories(self):
        """Specify the orbital factories to use"""
        self.srcOrbFactory = RadicalOrbitalFactory(self.srcAtomList, occupied=True, allSymmetry=True,
                                                   photo=self.photo)
        self.sinkOrbFactory = RadicalOrbitalFactory(self.sinkAtomList, occupied=False, allSymmetry=False,
                                                    photo=self.photo)
...
class RadicalOrbitalFactory(AtomBasedOrbitalFactory):
    ...
    """
    def __init__(self, atomObjList, occupied=False, allSymmetry=False, allKekule=True, photo=False):
    ...
    def prepareChainOrbital(orbital, parentOrb, occupied, photo):
        ...
        hasExtendedPotential = parentOrb.type == 'sigma' and \
            (not occupied) and \
            orbital.isLoneRadical()
        ...
        if not self.occupied and orb.type == 'sigma':
            for atom in orb.neighbor.GetAtoms():
                ...
                for potChainOrb in radicalOrbitalIter(atom, useSymmetries=False):
                    chainOrb = self.prepareChainOrbital(potChainOrb, orb, self.occupied, self.photo)
    ...
    def isValidOrbitalOccupation(orbital, occupied, photo):
        ...
        if occupied:
            if orbital.type == 'pi' and not photo:
                ...
                accept = (orbital.atom.GetAtomicNum() not in set([1, 6]) \
            ...
            if orbital.type == 'sigma':
                ...
                accept = (orbital.atom.GetAtomicNum() not in set([ 6]) \
                          and orbital.neighbor.GetAtomicNum() not in set([1])) \
                          or PERIODIC_ROW[orbital.atom.GetAtomicNum()] > 2 \
                          or orbitalInfo(orbital.neighbor)['nRadicals'] > 0 \
                          or OEBondIsInRingSize(orbital.atom.GetBond(orbital.neighbor), 3)
