from chemutils.CombiCDB.proposers.Util import (
    isBondDissociation,
    isReasonableOrbital,
    compatibleOrbitalPair,
)
from chemutils.CombiCDB.OrbitalInteraction import isPericyclic
from chemutils.CombiCDB.ReactionProposer import (
    BaseOrbitalPairFilter,
)
    ...
    def __call__(self, srcOrb, sinkOrb):
        for o in self.chained_orb_iter(srcOrb):
            ...
            if o.type == "sigma":
                if (
                    o.atom.IsInRing()
                    and o.neighbor is not None
                    and o.neighbor.IsInRing()
                ):
...
class PiBondBondDissociationFilter(BaseOrbitalPairFilter):
    ...
    def __call__(self, filledOrb, unfilledOrb):
        return not (
            isBondDissociation(filledOrb, unfilledOrb) and unfilledOrb.type == "pi"
        )
